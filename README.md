...
Direct Hermes CLI/API sessions use the same note, schedule, image, document,
conversion, search, and other configured capabilities without Zalo or Message Worker.
Messaging adds conversation authorization, queueing, and channel delivery;
it is not the capability execution layer. See [current execution contracts](./docs/03-architecture.md).
...
Current worker/feature inventory, descriptions and active/inactive defaults:
[worker and feature catalog](./docs/07-worker-feature-catalog.md).
...
| `rule/GIT.md` | Git workflow: develop first, then merge to main after it passes |
