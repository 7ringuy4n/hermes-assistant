...
do_stop_disabled_optionals() {
  ...
  if ! _env_active "${ENABLE_CLOUDDRIVE:-}"; then
...
do_compact() {
  # Daily memory housekeeping is a core task: run it regardless of whether the
  # optional media worker is installed. Fail soft when memory is unavailable.
  echo "==> compact (memory housekeeping) — silent"
  local mem="${MEMORY_URL:-http://127.0.0.1:8095}"
  curl -fsS -m 30 -X POST "${mem}/v1/compact" >/dev/null 2>&1 \
    || echo "WARN: memory compact unavailable at ${mem}" >&2
...
EOF
  ...
  # Daily memory housekeeping (compact staged drafts + reindex). Memory is a core
  # service, so this timer is always installed, not gated on the media worker.
  $sudo tee "${unit_dir}/assistant-compact.service" >/dev/null <<EOF
[Unit]
Description=Assistant daily memory compact
After=docker.service
[Service]
Type=oneshot
WorkingDirectory=${STACK_ROOT}
...
# Timers (backup/learn/stack-watch/compact; log-archive 30d; zalo-watch when ENABLE_ZALO=active)
...
ensure_grafana_admin_password() {
  ...
  # fallback. Idempotent: uses the OpenBao/.env value when real, else generates
  # one, writes it to .env (seeded by first-setup-openbao) and exports it here.
  _env_active "${ENABLE_GRAFANA:-}" || return 0
  [[ -f "${SCRIPTS_DIR}/ensure-grafana-admin.py" ]] || return 0
  local val
  val="$(STACK_ROOT="${STACK_ROOT:-$ROOT}" \
        ENABLE_GRAFANA="${ENABLE_GRAFANA:-}" \
        GRAFANA_ADMIN_PASSWORD="${GRAFANA_ADMIN_PASSWORD:-}" \
        python3 "${SCRIPTS_DIR}/ensure-grafana-admin.py" 2>/dev/null || true)"
  if [[ -n "$val" ]]; then
    export GRAFANA_ADMIN_PASSWORD="$val"
    _reset_grafana_admin_password "$val"
  fi
}

_reset_grafana_admin_password() {
  # Grafana applies the admin password only on first DB init; an existing
  # grafana_data volume keeps the old hash. Align the DB with the configured
  # secret so the credential is never the fallback.
  local pw="$1" c
  [[ -n "$pw" ]] || return 0
  c="$(docker ps -q \
  ...
  [[ -n "$c" ]] || return 0
  docker exec "$c" grafana-cli admin reset-admin-password "$pw" >/dev/null 2>&1 \
    || docker exec "$c" /usr/share/grafana/bin/grafana-cli admin reset-admin-password "$pw" >/dev/null 2>&1 \
    || true
}
