...
Direct Hermes → notes skill command → trusted local scope → Memory Worker.
Messaging → authenticated host scope → the same durable notes API.
Typed group/DM cross-scope access requires the operator admin; all registered
scopes are read-only. Date and scope constraints remain in SQL; complete
semantic predicates, including exclusions, are selected by the model and
validated against supplied candidate IDs. Uncertainty fails closed.
...
Explicit `origin.platform=hermes` schedules call `HERMES_RUN_URL`, even when a
local session identifier is present. They do not inject into Zalo or require
Message Worker. Unsupported platform values are rejected. Messaging schedules
retain their destination-bound verbatim/process execution.
...
reconnect timeouts. See [test/README.md](../test/README.md).
