...
Operational retention values are host configuration, not secrets. They default
to `BACKUP_RETENTION_DAYS=7`, `MEMORY_STAGED_RETENTION_DAYS=7`, and
`SESSION_IDLE_ROLLOVER_SECONDS=3600`, are loaded from this file and `.env`, and
can be changed without editing OpenBao. Existing OpenBao KV entries for the two
retention keys are purged during seed/update so one source remains.
...
# Daily backup rotation window (days).
BACKUP_RETENTION_DAYS=7
# Days a staged memory draft survives before housekeeping retires it.
...
# Start a new conversation session after this many idle seconds (1h).
SESSION_IDLE_ROLLOVER_SECONDS=3600
...
Run verification separately using [test/README.md](../../test/README.md).
