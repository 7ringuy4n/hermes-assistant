...
For the complete service/feature inventory, descriptions and fresh
active/inactive defaults, see [worker and feature catalog](./07-worker-feature-catalog.md).
...
Message/Zalo is optional for direct Hermes capability use. Notes need Memory
Worker, scheduled execution needs Schedule worker, and office rendering needs
Media worker; image/search use their configured capability routes. Disabling
messaging does not authorize disabling those capability dependencies.
...
| **Message / Zalo** | `message` or `zalo` | zalo-proxy + zalo-api; Telegram channel pack is currently a scaffold |
...
| CloudDrive mirror | `clouddrive` | rclone remote documents mirrored locally; explicit backup sync is a separate command |
