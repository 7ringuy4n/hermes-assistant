## 2026-09-15 23:00 +07 — Move `docs/GIT.md` into `rule/`; drop duplicate

- Removed the duplicate `docs/GIT.md`; `rule/GIT.md` (develop-first workflow) is
  now the single copy. Repointed references in `README.md`, `docs/README.md`,
  `AGENT_RULES.md`, and `.cursor/*` to `rule/GIT.md`.
- Kept `history/2026-09-14/production-gap-audit.md` (unique detailed
  root-cause/gap audit referenced by `docs/HISTORY.md` and
  `docs/07-worker-feature-catalog.md`).

## 2026-09-15 22:00 +07 — Invariant test strategy; test docs consolidated

- Combined `test/RULES.md` + `test/INVARIANTS.md` into `test/README.md`
  (verification contract + invariant registry); merged the gap-cases index
  (`test/cases/README-gap-cases.md`) in as well and removed the source files.
  References updated in `docs/`, `test/cases/`, and `test/scripts/`
  (`invariant_registry_unit`, `notes_atomic_cite_unit` now read `test/README.md`).
- New invariant units: `test/scripts/queue_fifo_recovery_unit.py`
  (INV-CONV-001 / INV-QUEUE-001/002: FIFO order, bounded capacity, claim/ack,
  worker-failure recovery with no loss/dup), `test/scripts/archive_member_path_unit.py`
  (INV-SEC-001/003/006/007: traversal/absolute containment, executable members
  excluded, member/byte caps), and `test/scripts/ssrf_guard_unit.py` (INV-SEC-002),
  plus `test/scripts/invariant_registry_unit.py` (registry coverage guard).
- `test/INVARIANTS.md` coverage updated: INV-CONV-001, INV-QUEUE-001/002,
  INV-SEC-001/003 now COVERED.
- Removed unused `test/fixtures/`, `test/NOTES.md`, and the corrupted
  `scripts/main/Apply-EdgeUpdate.ps1`; moved `test/SETUP.local.md` to local `rule/`;
  refreshed `test/REPORT.md`.
- `rule/AGENT_RULES.md` §33 now requires updating `test/REPORT.md` after test cases.
- Verification: local self-run 21/21; VPS offline suite 148/148 PASS.

## 2026-09-15 21:30 +07 — Archive path containment test; local progress file ignored

- Added `test/scripts/archive_member_path_unit.py` (INV-SEC-001/003/007) against
  the real `architect/tools/ingest/archive_media.py` guard: traversal/absolute
  member names never escape `dest_dir`, oversized members are skipped, and the
  member cap is enforced.
- `test/INVARIANTS.md`: INV-SEC-001 and INV-SEC-003 are now COVERED; INV-SEC-007
  evidence extended.
- `.gitignore`: ignore `history/task_on_progress.md` and the local `rule/` folder;
  `history/task_on_progress.md` is untracked and kept on disk as a local working
  file.
- Verification: local self-run 19/19; VPS offline suite 147/147 PASS; no
  crash-loops. See `history/task_on_progress.md`.

## 2026-09-15 21:00 +07 — Invariant-first test strategy: registry + SSRF guard

- Added `test/INVARIANTS.md`: maps platform invariants
  (INV-CONV/QUEUE/SEC/MEM/ROUTER/FAIL) to existing tests and lists the remaining
  coverage gaps, implementing the invariant-first coverage index.
- Added `test/scripts/ssrf_guard_unit.py`: exercises the real
  `architect/models/router-worker/websearch.py` `_validate_public_url` guard
  (scheme, URL credentials, non-standard port, non-global resolved address, and
  redirect re-validation).
- Added `test/scripts/invariant_registry_unit.py`: the registry must cover the
  required invariants and point at existing tests; the runner must report
  `running test case N/M`.
- Self-run (local, no VPS): 19/19 strategy-related units PASS.
- Operator rules updated locally under `rule/`: `HARDEN_RULE.md` merged into
  `AGENT_RULES.md` (§32), a Progress Tracking Rule added (§33), and `GIT.md`
  now states develop-first → main-after-pass. See `history/task_on_progress.md`.

## 2026-09-15 20:30 +07 — Grafana panels grouped by role and repacked

- The overview dashboard is organised into role sections: **Stack health**,
  **Traffic (Traefik)**, **Hardware**, **OmniRoute LLM usage**, and
  **Routing, queues & latency**. Panels sit side by side inside each section
  (wrap at 24 columns) instead of one per line.
- Traefik by-label series render as `{entrypoint="web"}`, `{service="…"}`, and
  `{router="…"}` in the legend.
- `test/scripts/monitor_metrics_unit.py` guards the section titles, the packed
  layout, and the legend formats. See `history/2026-09-15/README.md`.

## 2026-09-15 20:00 +07 — Grafana admin password fails closed

- `GRAFANA_ADMIN_PASSWORD` is no longer silently left on the Compose fallback
  (`changeme-set-in-env`). An idempotent `scripts/main/ensure-grafana-admin.py`
  (called by `run.sh` on `up`/`update`/`install` when `ENABLE_GRAFANA` is active)
  keeps a real secret in `.env`/OpenBao, and `run.sh` reconciles the Grafana DB
  with `grafana-cli admin reset-admin-password` because Grafana applies the env
  value only on first init.
- `check-security.sh` now authenticates the Grafana admin (`/api/user`) and fails
  when the credential is empty or the fallback, instead of only probing
  `/api/health`.
- `test/scripts/grafana_admin_password_unit.py` covers generation, idempotency,
  and the wiring. See `history/2026-09-15/README.md`.

## 2026-09-15 19:00 +07 — Grafana dashboards consolidated; routing/queue latency metrics

- Grafana now ships **two** dashboards: `assistant-logs` (all Loki panels; the
  `[flow]` file-pipeline logs moved here from `assistant-file-flow`) and
  `assistant-overview` (metrics + hardware; the duplicate OmniRoute row and the
  redundant `assistant-stack` panels were removed).
- `assistant-overview` gained a *Routing, queues & latency* row: request total /
  p95 latency (Traefik), LLM latency p50/p95, LLM error rate, LLM tokens,
  router selection, provider fallback, queue depth, Qdrant query latency, and
  schedule lag.
- New metrics: Traefik Prometheus metrics (`traefik_*` on entrypoint `:8082`),
  Router Worker `/metrics` (`router_worker_llm_*`, `router_worker_router_selection_total`,
  `router_worker_provider_fallback_total`), and stack-exporter
  `assistant_qdrant_query_seconds`, `assistant_queue_depth`, `assistant_schedule_lag_seconds`.
  Prometheus scrape jobs added for `router-worker` and `traefik`. The Router
  Worker image now ships `metrics.py` (an early build crash-looped on
  `ModuleNotFoundError`) and `/v1/classify` + `/v1/outbound` attempts are recorded
  in addition to the chat proxy path.
- See `history/2026-09-15/README.md`. LLM token counters reuse OmniRoute's
  `omnirouter_*_tokens_total` rather than duplicating them.

## 2026-09-15 18:20 +07 — env-driven retention, daily compact, idle session rollover

- `BACKUP_RETENTION_DAYS` and `MEMORY_STAGED_RETENTION_DAYS` moved out of OpenBao
  into host configuration (`.env` / `docs/config/DEFAULTS.md`, default 7) so the
  rotation window changes without editing the secret store. Existing OpenBao KV
  entries are purged on seed/update, and the backup/compact units load `.env`.
- Daily memory compact is core: `assistant-compact.timer` is always installed and
  `run.sh compact` no longer requires the optional media worker.
- The session service starts a new conversation session when the live session has
  been idle `SESSION_IDLE_ROLLOVER_SECONDS` (default 3600), archiving the old
  short-term context to the Memory Worker (`source` `session-rollover`) first.
- `test/scripts/session_idle_rollover_unit.py`, `test/scripts/compact_daily_unit.py`,
  and the updated `test/scripts/openbao_common_unit.py` cover the class.
- Fixed a stale `test/scripts/media_capability_skills_unit.py` assertion that still
  described the pre-change overlay-language policy; it now asserts the shipped
  English-only overlay contract in `image-runtime.json`.
- See `history/2026-09-15/README.md`.

## 2026-09-15 16:00 +07 — doc values match current configuration

- Corrected `LEARN_REQUIRE_APPROVE` in current docs: it is channel-derived and
  off (`0`/`inactive`) is what enables scan/midnight auto-ingest; Zalo submit is
  always pending (`docs/07-worker-feature-catalog.md`, `architect/tools/README.md`,
  `architect/tools/ingest/README.md`, `hermes/main/setup/README.md`,
  `test/cases/12-skills-auto-learn.md`).
- `docs/config/DEFAULTS.md`: `VISION_OCR_COMBO`→`OMNIROUTER_VISION_COMBO`,
  `EMBEDDING_MODEL`→`EMBED_MODEL`, and dropped non-existent `SKILLS_DIR`.
- Replaced the retired alias `model-router` with `router-worker`
  (`AGENT_RULES.md`) and removed a non-existent `HERMES_WRITE_SAFE_ROOT`
  reference (`hermes/main/skills/file-gen/SKILL.md`).
- See `history/2026-09-15/README.md`.

## 2026-09-15 15:30 +07 — environment switches accept active/inactive

- Reconciled boolean env switches that used a falsey set omitting `inactive`
  (so `inactive` was read as ON): `MESSAGE_TIMING_RECORD` (dispatcher,
  memory-worker), `ZALO_RATE_LIMIT`, `HERMES_MAX_ANSWERING`,
  `ZALO_HISTORY_POSTGRES`, `ZALO_SESSION_VALKEY`, `HERMES_WORKFLOW`/`ZALO_WORKFLOW`,
  and `ASSISTANT_STORE_URL`/`REDIS_URL`. Standard: on = `active|1|true|yes|on`.
- `test/scripts/env_switch_semantics_unit.py` covers the class.
- See `history/2026-09-15/README.md`.

## 2026-09-15 15:00 +07 — scope authz tests; learn approval wiring

- Added role-based scope tests: a group member reads only their group, a DM user
  only their DM, and only the operator admin may read another registered
  group/DM or `all`; schedules never cross a conversation. New
  `test/scripts/zalo_scope_authz_unit.py` and `test/cases/77-…`.
- Fixed ingest `LEARN_REQUIRE_APPROVE` parsing: the stack switch value
  `inactive` is now off (was treated as on). The compose default derives from the
  channel state, and installing `message`/`zalo` sets approval active.
- `test/scripts/learn_approval_unit.py` covers the Zalo always-pending submit and
  the scan auto/approve branches.
- See `history/2026-09-15/README.md`.

## 2026-09-15 14:00 +07 — quote-reply resend keeps quoted context

- Classify now treats a resend/repeat/re-show request as normal chat that refers
  to the quote, instead of routing by the quoted topic; the adapter keeps the
  `[Quoted message]` block through the live-search contract rebuild.
- Fixes `gửi lại` quote-replies that answered “no earlier content to resend”.
- See `history/2026-09-15/README.md`.

## 2026-09-15 — agent rules: no provider/model hardcoding

- `AGENT_RULES.md` §8 adds explicit prohibitions on hardcoding vendor/provider/
  model ids, capability allow-lists, and `*_FALLBACK` lists, and on detecting
  provider/infrastructure failures with natural-language phrase/marker lists.
  Capability comes from the configured catalog/combos; when no configured
  provider can serve a capability the code must fail soft (skip + inform), never
  substitute a literal model or fabricate a result.
- Adds §11.5 (configuration owns provider/model selection) and checklist items.

## 2026-09-15 13:15 +07 — hotfix: autosend guard import order

- Fixed a regression where `_as_autosend_turn_files` called
  `media_sent_in_turn` before importing it, raising `UnboundLocalError` on every
  turn (messages aborted, 15-minute queue timeout, “session interrupted”). The
  guard now runs after the autosend import; `autosend_unit` asserts the ordering.
- See `history/2026-09-15/README.md`.

## 2026-09-15 13:00 +07 — English-only image overlays

- Image overlays are always English now; an explicit request for another
  image-text language no longer overrides the default (only exact quoted wording
  is rendered verbatim). English overlays use a Latin money form (`VND`) instead
  of the `đ`/`₫` glyph.
- The flexible-layout gate drops its Vietnamese image-text directive and expects
  English copy for both images. Live verdict improved from 6 to 7 with
  `language_correct=true`; only diffusion text-rendering misspellings on the
  scheduled image keep `spelling_correct=false`.
- See `history/2026-09-15/README.md`.

## 2026-09-15 12:15 +07 — duplicate composed-image delivery and copy

- The autosend scan no longer re-sends an older same-thread image after the
  scheduled-fire shortcut already delivered that turn's composed image
  (`_as_autosend_turn_files` now honors `autosend.media_sent_in_turn`).
- Composed-image plans no longer render the same fact rows twice: top-level
  facts duplicated inside a panel are dropped so a shared region is drawn once.
- Live flexible-layout now yields distinct artifacts, a single left region,
  correct data and language; only minor model copy polish remains.
- Recorded that designed (non-trivial) data PDFs route through the async
  workflow by design; the case-39 slowness came from the job model choosing a
  local generation script instead of the file-gen/dispatcher path.
- See `history/2026-09-15/README.md`.

## 2026-09-15 11:00 +07 — visual oracle and composed-image planner

- The visual-PDF factual oracle no longer flags a correct measurement
  disclaimer (`không phải số đo từ trạm quan trắc`) as
  `open_meteo_model_mislabeled_measurement`.
- Composed-image structured planning now requests low reasoning and retries once
  with a bounded larger budget when a reasoning model returns
  `finish_reason=length`, so a truncated plan no longer loses the image.
- Latency gate 17 remains provider/context bound: the shared DM session carried
  ~92k input tokens (116 messages); a fresh session completes in ~1.5 s.
- See `history/2026-09-15/README.md`.

## 2026-09-15 10:30 +07 — vision provider blocked; remaining-suite harness fix

- Image analysis (`hình gì`) is blocked by OmniRoute's VISION-BRIDGE: the
  `vision-ocr` combo lost its vision-capable member and the bridge reports
  `No vision-capable provider connected`, then `Vision API error 401` once a
  member is restored. Chat calls to the same providers succeed; OmniRoute's
  `model_capabilities` table is empty. No host-side phrase list or hardcoded
  model fallback was retained — the host skips and informs on an empty read.
- `test/scripts/zalo_tn_remaining_suite_remote.py` writes its report via an
  atomic temp-file replace, so a prior sudo-backed run cannot leave a root-owned
  `/tmp/hs-suite-report.json` that aborts the remaining suite.
- Live case-index run over the 11 previously-unpassed gates: 8 PASS, 2 blocked
  by the vision provider (39, flexible-layout), 1 latency SLO FAIL (17).
- See `history/2026-09-15/README.md`.

## 2026-09-15 — safe note mutations and composed-image delivery

- Bulk destructive note mutations now require an explicit confirmation. A
  single model selection pass can no longer silently delete or overwrite
  several durable notes; the host previews the exact matched records and applies
  only the previewed ids after a standalone confirmation token.
- Delete and update note operations now report their own outcome instead of the
  generic “Đã lưu ghi chú” save line.
- Classify `notes` and the notes record-selection prompt now preserve exclusion
  predicates and forbid `match_all` when a request names any qualifier.
- The Zalo plugin now ships its own `artifact_delivery.py`. The Hermes container
  mounts `hermes/main/plugins` read-only over `/opt/data/plugins`, so the
  runtime copy performed by `sync-zalo-plugins.sh` was shadowed and the shortcut
  direct-artifact path raised `ModuleNotFoundError` (composed image reported
  “Hiện chưa tạo được file này”).
- See `history/2026-09-15/README.md`.

## 2026-09-14 16:21 +07 — authorized promotion with known failures

- Explicit user override permits develop/main MRs despite failed gates; no
  production certification or VPS deployment is implied.
- Startup completion fix passes the latest complete 134/134 offline run.
  VPS totals are 15/26 PASS, 11 unpassed (4,5,6,7,8,12,16,18,21,22,25).
- Latest PDF case4 remains FAIL at240s; greeting remains10,898ms vs5,000ms.
  Native-client quoted-file delivery remains unverified/failed.
- Correct negated model-attribution text is no longer an oracle false positive.
  Continuous-message checks require all semantic booleans and exact native ACKs;
  these latest observer changes have focused checks, not fresh full/live PASS.
- Safe-path HTML submission detours/repeated PDF rendering remain unresolved.

## 2026-09-14 16:01 +07 — startup-buffered work is not terminal (candidate)

- Actual PDF logs prove a workflow can complete/fail while its gateway event
  is still buffered for startup replay. Include exact pending events in idle
  checks and fence only the owned event/cancelled replay on timeout.
- Delayed replay, unreplayed timeout, foreign type and actual adapter-method
  cancellation regressions pass locally. Normal deployment pending gate23.
- Attribution refinement offline134/134 passes; PDF remains FAIL, totalVPS
  14/26passed,12unpassed. No timeout extension, combo change or promotion.

## 2026-09-14 15:54 +07 — native quote concurrency PASS (candidate)

- Gate19 passes two source/channel-bound native quoted ACKs, exact sentences,
  no crossover and terminal settlement in48.89s. Injected contexts are not
  native-client input proof. Fresh VPS14/26 passed;12unpassed. No promotion.
- File-gen attribution refinement validates and is normally deployed; fresh
  PDF/full offline reruns running. Note-locale observer now requires actual
  scoped storage/date/generated title/requested content and exact owned cleanup,
  not merely a localized confirmation. Live note rerun pending.

## 2026-09-14 15:51 +07 — policy-refusal observer and quote proof (candidate)

- Gate 14 passes two checks, semantic 10/10. Include the host's final policy
  response despite its `gate` tag; require exact source ACK, terminality and
  no extra attachments. No quota/skip PASS or global draft oracle.
- Fresh 13/26 VPS passed; 13 unpassed. Grounding candidate offline 134/134
  passed, but PDF remains FAIL (266s vs240s and unsupported measurement/sunshine
  claims). Staged refinement addresses all model-data labels and `is_day`.
- Quote concurrency observer requires actual cached CLI identities, exact
  replies, acknowledged `quoted=true` and terminality. Injected context is
  explicitly not a native-client input quote. Live rerun pending.

## 2026-09-14 15:42 +07 — safe live gates and grounded PDF copy (candidate)

- Gates 17/26 and 13/26 pass private env cleanup and isolated real OpenBao
  CAS rotation, actual load/scrub and temporary consumer checks. Live KV,
  token, files and consumer env remain unchanged; fixtures are cleaned up.
- VPS total 12/26 passed, 14 unpassed. Gate 4 fails at 240s and delivers late
  (~366s); native review finds a WMO 55 condition mistranslation. Do not count
  late ACK or attractive layout as PASS. Keep the timeout unchanged.
- Add known factual-caption regression; file-gen uses verified provider code
  legends, honest model/observation attribution and scope-limited retrieval.
  Normal skill deployment complete; fresh PDF/full offline reruns pending.

## 2026-09-14 15:09 +07 — source-bound deferred notes (candidate)

- Fresh deployed verification: 134/134 offline entries and gate 24/26 pass
  original-source notes, explicitly requested content, terminal no-fire-response
  and exact owned cleanup. Fresh VPS progress is 10/26; 16 remain unpassed.
- PDF observer now identifies the content-bound original-source ACK, reviews all
  pages in one evaluator call and rechecks late extras. Quota/skip is not PASS;
  source names or unrelated logs cannot establish document defects. Live rerun
  pending. Quoted conversion fixtures now pass their explicit original source.

- Isolate concurrent pending notes by original chat/source; unrelated turns
  cannot consume or clear them. Retain authenticated selector/admin context
  and host-owned source provenance for deferred writes. Actual-function
  regression passes; normal plugin sync and replica recreation complete.
- Notes prompts distinguish explicitly requested user identifiers from
  internal runtime markers and keep short titles separate from summaries.
  Updated skill validates. Fresh offline/background reruns remain pending.
- Isolated SQL/vector compact gate 15/26 passes; intervening offline index
  passes 134/134. Fresh VPS total: 9/26 passed, 17 unpassed. No promotion yet.

## 2026-09-14 14:58 +07 — vector index and fresh delivery verification (candidate)

- Deterministic restart-stable UUID point identities, finite numeric embedding
  validation, and compact counts based on acknowledged upserts. Configured
  embedding failure no longer reports indexing success; inactive optional
  backends report no indexing. Existing legacy points are preserved. Five
  production-function regressions pass locally; live/full verification pending.
- Gate 20/26 passes source-bound concurrency plus semantic/native DOCX review.
  Gate 5/26 fails the unchanged SLO (10,898 ms versus 5,000 ms); no waiver.

## 2026-09-14 — original-source conversion delivery (candidate)

- Conversion requests retain the host-provided original source and channel
  through their delivery callback; sourceless messaging conversions are rejected
  before rendering. Standalone conversions remain destination-free. Four actual
  publication/binding regressions pass, including atomic batch failure.
- The validated conversion skill can convert and send straightforward authorized
  outputs in one worker call, without an extra model-selected send step. Requests
  requiring inspection/editing may still stage first. Fresh live gates remain open.

## 2026-09-14 14:35 +07 — model-owned document creation and exact shortcut paths (candidate)

- Clean DM/group concurrency exposed a silent DM draft: the host selected a
  display filename instead of its private renderer path, and printed creation
  instructions as document content. Both regression invariants fail before fix.
- Prefer the exact agent-mount artifact path and announce failed shortcut
  delivery; never substitute another conversation's output. Natural file
  requests now reach model-owned file-gen instead of implicit literal printout.
  The shared renderer and standalone file generation remain available.
- Focused regressions pass. Full offline and live concurrency reruns remain
  required on this latest candidate; no conditional promotion yet.

## 2026-09-14 14:27 +07 — queued text compatibility and transport verification (candidate)

- Preserve legacy text-only events without synthesizing artifact source IDs;
  original-source delivery context remains mandatory for messaging artifacts.
- Greeting verification requires one durable source-correlated transport ACK.
  The live greeting passes; all 134 offline index entries pass on current fixes.
  Seven of 26 VPS gates are freshly passed; native quote delivery and the other
  live gates remain open. No new promotion has been performed.

## 2026-09-14 14:15 +07 — terminal active antivirus verdicts (candidate)

- Security Manager no longer treats an asynchronous pending scan as CLEAN.
  Enabled antivirus waits for an explicit terminal verdict and rejects errors,
  outages and timeout even under generic fail-open policy. Eight focused
  oracle/helper regressions pass; rebuilt VPS gate 9/26 passes 5/5, and EICAR
  is blocked by actual antivirus with YARA disabled. Full release gates remain open.
- Scanner verification requires full typed verdicts rather than matching the
  word `infected`; owned temporary files avoid stale or shared test fixtures.

## 2026-09-14 14:03 +07 — source receipts for directly queued artifact replies (candidate)

- Directly queued document replies now consult their exact original-source
  Dispatcher ACK, just like isolated workflows, rather than relying on a
  sender-local media marker. Eleven actual-method regressions pass, including
  previous-source and missing-binding isolation. Fresh live gates remain required.
- Concurrency verification fails immediately when an extra acknowledged result
  makes terminal single-deliverable PASS impossible; no evidence is filtered out.
- The fresh concurrency run exposed a group document sent as `user` with an
  empty source. Queue execution now supplies its claimed delivery envelope
  independently of classification; Dispatcher rejects a missing source before
  sending. The failing regression passes after the fix. Live reruns remain open.
- See `history/2026-09-14/production-gap-audit.md` for the reproduced failure.

## 2026-09-14 13:38 +07 — operator-authorized promotion with incomplete live gates

- The operator explicitly requests merge requests and merging the current
  improvement set into develop and main despite the reported incomplete gate.
  This is a one-time promotion override, not a passing test result.
- Latest evidence: 134/134 offline suite files pass; eight conversational
  conversion cases and focused document/scoped-record checks pass. Four of
  26 indexed VPS gates have a fresh PASS. Document concurrency fails
  `FAIL_TERMINAL_DELIVERY_DOCUMENT`; 21 other indexed gates await fresh runs.
  Native quote bubbles and broader capability/lifecycle checks remain open.
- Keep failed results and production-gap history intact. Main promotion uses
  a selected release branch, not a wholesale develop merge. No VPS update is
  implied by the merge authorization.

## 2026-09-14 08:15 +07 — scoped records and visual document integrity (candidate)

- Bound antivirus multipart requests, concurrent parsing, scan queue/bytes,
  retained sessions and per-session files; expire only completed verdicts,
  sanitize quarantine filenames and preserve worker availability after errors.
  Ten capacity regressions and fresh live capacity/scanner checks (3/3 and 6/6)
  pass; the complete release matrix remains open.
- Publish converted page/slide batches atomically; storage/engine outages return
  bounded errors without partial public output. Reject empty test selections.
- Preserve original document requests separately from classifier suggestions.
  Late gateway replies retain their original isolated source/receipt binding
  after workflow cleanup and cannot borrow the next request's active turn.
  The fresh live PPTX failure prompted these fixes; reruns remain required.
- Preserve typed result-only document delivery before and after artifact ACK:
  suppress private draft-review chatter, never recover unrelated conversation
  text as a document result, and report localized delivery failures. Cancel
  timed-out workflow agent tasks before releasing their claim. Document jobs
  use the canonical 900-second review budget; ordinary jobs retain their budget.
  Live harnesses check current-replica session leases in addition to queue ACKs.
- Typed document jobs no longer discover unrelated artifacts by shared-directory
  timestamps. Eight binding/result-only regressions and the latest deployed
  offline index pass (134/134). Fresh PDF and PPTX deliver one requested file
  each; every native page/slide passes visual and request-scope review and
  selected bytes match bridge echoes. Broader release gates remain open.

- Correct optional antivirus/CloudDrive/sandbox lifecycle checks to recognize
  canonical `active`. Antivirus activation now enables required scanning in
  both Hermes and Dispatcher; unavailable, unfinished and failed scans cannot
  silently authorize parsing or delivery. Isolate inbound verdicts per file.
  Eight activation regressions and twelve real conversion-engine cases pass,
  including PDF/PPTX-to-image. Full fresh release gates remain open.

- Fix the five isolated production delivery regressions and expand coverage to
  nine Dispatcher, eleven shared-protocol and six real-adapter method cases.
  Scope content identity by original source/conversation, confine display names,
  retain explicit destinations, scan before reservation, require bridge IDs,
  and distinguish pending wire sends from acknowledged receipts. Ambiguous
  wire results are quarantined instead of silently resent. Fresh live gates
  remain required; these local passes do not authorize promotion.
- Introduce token-checked Session receipt transitions and share one protocol
  implementation between Dispatcher and the plugin. Document jobs bind delivery
  to original metadata and consult acknowledged receipts before final text.
  Translate native preview engine failures/timeouts into bounded API errors.

- Add quote-reply file-conversion regressions: acknowledged PPTX-to-PDF and
  all-sheet workbook-to-DOCX, with quoted-only inputs and exact source-content
  checks. Safely resolve staged quoted office files as files, not images;
  unrelated recent-file recall cannot override an explicit quote.
- Formatted authoring now requires a private native-text/page-image preview
  before selected-artifact delivery. Fix native presentation shade opacity and
  preserve opaque asset filenames during markdown parsing. These candidate
  changes do not declare the still-pending live release gates PASS.
- Add shared-worker `file-convert`: native Office-to-PDF, document page/slide
  images, image-to-PDF, and explicitly disclosed editable content reflow.
  Ten real-engine conversion regressions pass; conversational release gates
  remain pending. Source and intermediate outputs are never sibling deliveries.
- Standalone schedule handoff uses native asynchronous Hermes run admission
  and execution idempotency, rather than waiting for a model turn on a short
  chat-completion transport timeout. An actual short-delay fire persists its
  isolated direct note; broader conversational integration remains required.

- Preserve complete semantic note predicates, remove implicit query broadening,
  and fail closed on uncertain or oversized selection.
- Add typed operator-admin group/DM record access and read-only registered-scope
  lookup; schedule audit identities no longer grant conversation visibility.
- Keep document assets private, translate shared media aliases, reject failed
  image resources, embed native office visuals, and publish rendered files atomically.
- Fix both already-delivered document claim paths to suppress older sidecars.
  Two live Zalo exclusion cases and five live scoped core cases pass.
  Fresh PDF/PPTX content-quality gates fail; no conditional promotion occurs.
- Evidence and remaining work: `history/2026-09-14/README.md`.
- Add direct Hermes note execution, destination-free office generation, private
  draft publication, and explicit Hermes schedule handoff independent of Zalo.
- Suppress bundled PowerPoint/PPTX backfill and route presentations through the
  shared renderer. Fresh document turns send one requested file each, but
  missing icons/unrequested content still prevent release PASS.
- Rename the source component to memory-worker while preserving the `memory`
  deployment key, service URLs, databases, and data volumes. Update current
  architecture, capability guidance, setup paths, and affected regression paths.
