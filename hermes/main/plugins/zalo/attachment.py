...
def _is_local_attachment_path(value: str) -> bool:
    """A quoted shared-volume file, never an arbitrary filesystem path."""
    path = str(value or "").strip().strip("\"'").replace("\\", "/")
    parsed = PurePosixPath(path)
    return bool(any(path.startswith(root) for root in _LOCAL_MEDIA_ROOTS)
                and ".." not in parsed.parts
                and parsed.suffix.lower() in TEXT_EXTS + OCR_EXTS + OFFICE_EXTS + AV_EXTS + ARCHIVE_EXTS)
...
def _local_media_path_from_text(text: str) -> str:
    ...
    for chunk in str(text or "").replace("\n", " ").split():
        ...
        if _is_local_attachment_path(candidate):
...
def _media_dict_from_local_path(path: str) -> Dict[str, Any]:
    ...
    return {
        "kind": "image" if "." + ext in _IMAGE_FILE_EXTS else "file",
        "url": clean,
        "localPath": clean,
...
def merge_inbound_quote_media(
    ...
    if isinstance(media, dict):
        ...
        if url.startswith("http") or _is_local_attachment_path(url):
            return media, m
    ...
    if isinstance(raw_q, dict):
        ...
        if isinstance(qmedia, dict):
            ...
            if qurl.startswith("http") or _is_local_attachment_path(qurl):
                nm = dict(m)
                nm["media"] = qmedia
                return qmedia, nm
    return media, m
