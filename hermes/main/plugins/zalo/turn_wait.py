...
PendingGet = Callable[[], Sequence[Any]]
...
async def wait_thread_idle(
    ...
    *,
    ...
    arm_s: float = ARM_S,
    pending_get: Optional[PendingGet] = None,
) -> bool:
    ...
    wait until it clears. Gateway startup-buffered events count as active:
    the initial callback returning does not mean the replayed agent completed.
    """
    ...
    while True:
        ...
        if pending_get is not None:
            kind={'user':'dm','private':'dm'}.get(str(thread_type).lower(),str(thread_type).lower())
            for event in pending_get():
                source=getattr(event,'source',None)
                platform=getattr(source,'platform',None)
                if (str(getattr(platform,'value',platform))=='zalo'
                    and str(getattr(source,'chat_id',''))==str(thread_id)
                    and (not kind or str(getattr(source,'chat_type','')).lower()==kind)):
                    active=True
                    break
        if active:
            saw_active = True
