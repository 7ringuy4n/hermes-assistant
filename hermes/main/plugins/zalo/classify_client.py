...
def normalize_task_details(
    ...
    for i, item in enumerate(rows):
        ...
        out_type = str(body.get("output_type") or body.get("file_format") or "").strip().lower()
        if out_type in {"text", "txt."}:
            out_type = "txt"
        if out_type not in {"image", "pdf", "txt", "docx", "xlsx", "csv", "md", "pptx"}:
            out_type = ""
        row_out = {
            "execution_class": cls,
            "task_type": typ,
        ...
        if body.get("artifact_operation") in {"create", "convert", "read"}:
            row_out["artifact_operation"] = body["artifact_operation"]
...
def plan_allows_office_shortcut(plan: dict[str, Any] | None) -> bool:
    """Natural file requests need model authoring, not literal instruction printout.

    A file task label does not prove its instructions are finished document
    content. The file-gen skill and direct shared renderer API remain available;
    do not infer a literal body from prose or a classifier's optional flag.
    """
    return False
...
def plan_is_image_analyze_chat(
    ...
    if src.get("ok") is False:
        return False
    if src.get("artifact_operation") == "convert":
        return False
    parts = [str(x).strip() for x in (src.get("instructions") or []) if str(x).strip()]
...
def _coerce_schedule_selector(raw: Any) -> dict[str, Any] | None:
    ...
    scope = str(raw.get("scope") or "current").strip().lower()
    scope_ref = str(raw.get("scope_ref") or "").strip()[:256]
    ...
    if raw_index is not None and str(raw_index).strip() != "":
        ...
        if n >= 1:
            list_index = n
    if not name and not content_hint and not time_hint and list_index is None and scope == "current" and raw.get("view") != "detail":
        return None
    out: dict[str, Any] = {
        "id": None,
        "name": name,
...
def _coerce_note_selector(raw: Any) -> dict[str, Any] | None:
    ...
    semantic_query = str(raw.get("semantic_query") or "").strip()[:1000]
    scope = str(raw.get("scope") or "current").strip().lower()
    scope_ref = str(raw.get("scope_ref") or "").strip()[:256]
    ...
    if view not in {"list", "detail", "count"}:
        view = "list"
    if not note_id and not query and not semantic_query and not date_from and not date_to and not tags and not match_all and scope == "current":
        return None
    return {
        "id": note_id,
        "query": query,
...
def normalize_plan(data: dict[str, Any] | None, text: str, timezone: str) -> dict[str, Any]:
    ...
    plan = {
        ...
        "artifact_operation": src.get("artifact_operation") if src.get("artifact_operation") in {"create", "convert", "read"} else None,
