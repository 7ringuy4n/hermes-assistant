...
class ZaloAdapter(BasePlatformAdapter):
    ...
    def set_message_handler(self, handler) -> None:
        ...
        self._as_gateway_handler_owner = getattr(handler, "__self__", None)

        async def _capture_terminal_response(event):
            if getattr(event, "_as_gateway_cancelled", False):
                return None
    ...
    def _as_pending_gateway_events(self):
        owner = getattr(self, "_as_gateway_handler_owner", None)
        pending = getattr(owner, "_startup_restore_queue", ())
        if not isinstance(pending, (list, tuple)):
            raise RuntimeError("gateway startup queue unavailable")
        return tuple(pending)

    def _as_cancel_buffered_gateway_event(self, event):
        # Fence only this original event. A replay already popped from the
        # queue will still be suppressed by the handler wrapper above.
        event._as_gateway_cancelled = True
        owner = getattr(self, "_as_gateway_handler_owner", None)
        pending = getattr(owner, "_startup_restore_queue", None)
        if isinstance(pending, list):
            pending[:] = [item for item in pending if item is not event]
    ...
    def _zalo_rate_limit_cfg(self):  # ASSISTANT_RATE_LIMIT_v4
        ...
        if raw in {"0", "off", "false", "no", "inactive"}:
            return 0, 10.0
        try:
            n = int(raw)
        except ValueError:
    ...
    def _as_mark_pending_note_persist(
        ...
        *,
        ...
        source_context = getattr(self, "_as_source_message_id", None)
        source_id = str(source_context.get() or "") if source_context is not None else ""
        key = (tid, source_id) if source_id else tid
        self._as_pending_note_persist[key] = {
            "sender_id": str(sender_id or ""),
            ...
            "note_selector": dict(note_selector or {}),
            "admin_ids": list(admin_ids or []),
        }

    def _as_mark_pending_note_mutation(
        self,
        *,
        thread_id: str,
        thread_type: str,
        ...
        if not tid:
            return
        self._as_pending_note_mutation = getattr(self, "_as_pending_note_mutation", {}) or {}
        self._as_pending_note_mutation[tid] = {
            "thread_type": str(thread_type or "user"),
            "sender_id": str(sender_id or ""),
            "user_text": str(user_text or ""),
            "plan": dict(plan or {}),
            "ids": [str(value) for value in (ids or []) if str(value).strip()],
            "ts": time.time(),
        }

    def _as_clear_pending_note_mutation(self, thread_id: str) -> None:
        try:
            from .turn_wait import real_thread_id
        except ImportError:
            from turn_wait import real_thread_id  # type: ignore
        tid = real_thread_id(str(thread_id or "").strip())
        pending_map = getattr(self, "_as_pending_note_mutation", None)
        if tid and isinstance(pending_map, dict):
            pending_map.pop(tid, None)

    def _as_take_pending_note_mutation(self, thread_id: str, text: str) -> dict | None:
        """Return the pending bulk mutation only for an explicit confirmation."""
        try:
            from .turn_wait import real_thread_id
        except ImportError:
            from turn_wait import real_thread_id  # type: ignore
        tid = real_thread_id(str(thread_id or "").strip())
        pending_map = getattr(self, "_as_pending_note_mutation", None)
        if not tid or not isinstance(pending_map, dict):
            return None
        pending = pending_map.get(tid)
        if not isinstance(pending, dict):
            return None
        if time.time() - float(pending.get("ts") or 0) > 600:
            pending_map.pop(tid, None)
            return None
        if not self._as_note_mutation_confirm(text):
            pending_map.pop(tid, None)
            return None
        pending_map.pop(tid, None)
        return pending
    ...
    @staticmethod
    def _as_note_mutation_confirm(text: str) -> bool:
        """True only for a standalone explicit confirmation token."""
        import unicodedata
        ...
        return folded in {"xac nhan", "confirm", "dong y", "yes"}

    async def _as_run_confirmed_note_mutation(
        self,
        *,
        pending: dict,
        thread_id: str,
        ...
        plan["confirmed"] = True
        user_text = str(pending.get("user_text") or "")
        result = await execute_note_plan_async(
            plan,
            thread_id=str(thread_id),
            thread_type=str(thread_type),
            sender_id=str(sender_id),
            admin_ids=self._zalo_admin_uids(),
            override_ids=list(pending.get("ids") or []),
        )
        ...
        if result.get("success"):
            ...
            if action == "delete":
                ...
                body = self._as_ux_line(
                    "ZALO_NOTES_UPDATED_MSG",
                    ("notes", "updated"),
                    "Updated {count} note(s).",
                    user_text=user_text,
                )
        ...
        try:
            ...
            logger.warning("[zalo] confirmed note mutation reply failed thread=%s", thread_id)
    ...
    async def _as_persist_deferred_notes(
        ...
        source_context = getattr(self, "_as_source_message_id", None)
        source_id = str(meta.get("source_message_id") or (
            source_context.get() if source_context is not None else "") or "")
        key = (tid, source_id) if source_id else tid
        if isinstance(pending_map, dict) and tid:
            pending = pending_map.pop(key, None)
            if pending is None:
                pending = pending_map.pop(tid, None)  # legacy unbound text events
        ...
        if not notes:
            return "" if pending.get("silent") is True else (cleaned or content)
        for note in notes:
            # Provenance is host-owned, never an LLM text marker or claim.
            note["metadata"] = {**dict(note.get("metadata") or {}),
                                "source_message_id": str(pending.get("source_message_id") or "")}
        plan = {
            "skill_action": "create",
            "notes": notes,
            "note_selector": dict(pending.get("note_selector") or {}),
        }
        result = await execute_note_plan_async(
            plan,
            thread_id=tid,
            ...
            sender_id=sender_id,
            admin_ids=list(pending.get("admin_ids") or []),
        )
    ...
    async def _as_schedule_lifecycle(
        ...
        *,
        ...
        thread_type: str,
        plan: dict,
        authorized_rows: list | None = None,
    ) -> bool:
        action = str(plan.get("skill_action") or "").strip().lower()
        try:
        ...
        rows = authorized_rows if authorized_rows is not None else schedules_for_thread(thread_id, thread_type)
    ...
    async def _as_run_host_media_shortcut(
        ...
        *,
        ...
        if shortcut_ok(shortcut):
            ...
            try:
                artifact_path = str(
                    (shortcut or {}).get("hermes_path")
                    or (shortcut or {}).get("path")
                    or (shortcut or {}).get("file") or ""
                )
                if artifact_path:
                    ...
                    if artifact_available:
                        ...
                        if artifact_claimed:
                            ...
                            meta = {
                                "as_skip_autosend": True,
                                "as_claimed": True,
                                "delivery_claim_path": artifact_path,
                                "thread_type": (
                                    "group"
                            ...
                            if artifact_delivered:
                                ...
                                self._as_autosend_file_unclaim(artifact_path, str(thread_id))
                        else:
                            logger.warning(
            ...
            if not artifact_delivered:
                try:
                    ...
                    logger.warning("Zalo: shortcut failure notice could not be delivered", exc_info=True)
            try:
                from .session_memory import append_turn
            except ImportError:
                from session_memory import append_turn  # type: ignore
    ...
    async def _as_try_workflow_submit(
        ...
        *,
        ...
        current = strip_prior_for_classify(text) or str(text or "").strip()
        # A pending bulk note mutation executes only on an explicit confirmation
        # token, before classification can reinterpret the short reply.
        confirmed_mutation = self._as_take_pending_note_mutation(str(thread_id), current)
        if confirmed_mutation is not None:
            await self._as_run_confirmed_note_mutation(
                pending=confirmed_mutation,
                thread_id=str(thread_id),
                thread_type=str(thread_type),
                sender_id=str(sender_id),
            )
        ...
        if should_defer_note_persist(plan, current):
            self._as_mark_pending_note_persist(
                ...
                note_selector=plan.get("note_selector"),
                admin_ids=self._zalo_admin_uids(),
            )
        ...
        if plan_is_note(plan) and not schedule_fire:
            ...
            note_selector = plan.get("note_selector") if isinstance(plan.get("note_selector"), dict) else {}
            ...
            if result.get("success") and action == "lookup":
                ...
                if error == "confirm":
                    self._as_mark_pending_note_mutation(
                        thread_id=str(thread_id),
                        thread_type=str(thread_type),
                        sender_id=str(sender_id),
                        plan=plan,
                    ...
                    lead = self._as_ux_line(
                        ...
                        ("notes", "confirm"),
                        "This affects {count} note(s). Reply exactly CONFIRM to proceed.",
                        user_text=current,
                    ).replace("{count}", str(int(result.get("count") or 0)))
                    body = f"{lead.rstrip()}\n{candidates}" if candidates else lead
                elif error == "ambiguous" and candidates:
                    lead = self._as_ux_line(
        ...
        context = {
            ...
            "plan": plan,
            "attachments": list(media_urls or []) if plan.get("artifact_operation") == "convert" else [],
        }
        if plan.get("task_hint") == "schedule" and not schedule_fire:
            ...
            record_selector = plan.get("schedule_selector") if isinstance(plan.get("schedule_selector"), dict) else {}
            record_rows = None
            if skill_action in {"list", "inspect", "show", "status", "delete", "pause", "resume", "update", "run_now", "run"} or task_type in {"list_schedule", "delete_schedule", "pause_schedule", "resume_schedule", "update_schedule", "run_schedule"}:
                try:
                    from .schedule_client import schedules_for_record_scope
                    from .channels_client import extract_target_group_ref
                except ImportError:
                    from schedule_client import schedules_for_record_scope
                    from channels_client import extract_target_group_ref
                record_selector = dict(record_selector)
                legacy_ref = extract_target_group_ref(current, plan)
                if legacy_ref and record_selector.get("scope", "current") == "current":
                    record_selector.update(scope="group", scope_ref=legacy_ref)
                try:
                    if record_selector.get("scope") == "all" and skill_action not in {"list", "inspect", "show", "status"} and task_type != "list_schedule":
                        raise ValueError("all_scope_read_only")
                    record_rows = schedules_for_record_scope(record_selector,
                        thread_id=str(thread_id), thread_type=str(thread_type),
                        is_admin=self._as_is_zalo_admin(sender_id))
                except (ValueError, PermissionError):
                    msg = self._as_ux_line("ZALO_RECORD_SCOPE_DENIED_MSG", ("records", "scope_denied"),
                        "Cannot access that conversation's records.", user_text=current)
                    await self._as_gate_announce(thread_id, thread_type, msg)
                    return True
            if skill_action in {"list", "inspect", "show", "status"} or task_type == "list_schedule":
                ...
                if not schedule_enabled():
                    try:
                        ...
                        pass
                    return True
                rows = record_rows if record_rows is not None else schedules_for_thread(target_tid, thread_type)
                selector = plan.get("schedule_selector") if isinstance(plan.get("schedule_selector"), dict) else {}
                if selector.get("view") == "detail":
                    ...
                    base = format_schedule_list_lines(match_schedules_by_selector(rows, selector))
            ...
            if skill_action == "delete" or task_type == "delete_schedule":
                ...
                if selective:
                    ...
                    # Delete only the authorized candidate set, not a new wider
                    # lookup that could cross a typed conversation boundary.
                    for row in rows:
                        sid = str(row.get("id") or "").strip()
                        if sid:
                            data = delete_schedule(sid)
                            if data.get("ok") or data.get("deleted") == sid:
                                deleted.append(sid)
                try:
                    if deleted:
        ...
        if has_image_attachment:
            ...
            explicit_office = bool(
                plan_output_type(plan) and (plan.get("artifact_operation") == "convert" or _instruction_is_office_file_body(ins_parts))
            )
    ...
    async def _as_run_workflow_job(self, job: dict, wid: str) -> None:
        ...
        try:
            ...
            from job_prompt import workflow_instruction  # type: ignore
        try:
            instruction = workflow_instruction(instruction, ctx.get("task"), ctx.get("attachments"),
                                               {key: ctx.get(key) for key in ("thread_id", "thread_type", "source_message_id")},
                                               original_request=ctx.get("original_request"))
        except (OSError, ValueError):
            logger.exception("Zalo: workflow execution contract unavailable")
            fail_job(jid, "execution_contract_unavailable")
            return
        tid = str(ctx.get("thread_id") or "")
        ...
        document_jobs = getattr(self, "_as_document_jobs", None)
        if not isinstance(document_jobs, dict):
            document_jobs = self._as_document_jobs = {}
        if defer_sidecars:
            document_jobs[iso] = {"thread_id": tid, "thread_type": zalo_tt, "source_message_id": source_message_id}
            # Preserve the typed result-only contract beyond workflow cleanup.
            # Final gateway text is not evidence of artifact delivery.
            self._as_turns[iso]["artifact_only"] = True
        watch = (
            None
            if defer_sidecars
        ...
        try:
            ...
            idle = await self._as_wait_thread_idle(
                ...
                timeout_s=(self._as_env_float("ZALO_WORKFLOW_DOCUMENT_TIMEOUT_S", 900.0, 30.0, 900.0)
                           if defer_sidecars else None),
            )
            if not idle:
                # As in the durable inbound queue, fence the separate agent
                # task before releasing this job's claim or artifact window.
                self._as_cancel_buffered_gateway_event(event)
                await self.cancel_session_processing(self._event_session_key(event))
                raise RuntimeError("workflow agent session did not become idle")
            ...
            if defer_sidecars:
                delivered = self._as_job_already_sent_file(iso)
            recovered = False
            if defer_sidecars and idle and not delivered:
                failure = self._as_ux_line(
                    "ZALO_ARTIFACT_DELIVERY_FAILED_MSG", ("artifacts", "delivery_failed"),
                    "The requested file could not be delivered. Please try again.",
                    user_text=str(ctx.get("original_request") or instruction),
                )
                await self.send(iso, failure, metadata={
                    "as_skip_autosend": True, "skip_outbound_filter": True,
                    "as_skip_session_memory": True, "source_message_id": source_message_id,
                    "thread_type": zalo_tt,
                })
            if not defer_sidecars and not delivered and not self._as_job_already_sent_file(iso):
            ...
            try:
                ...
                await self.send(iso, msg, metadata={
                    "thread_type": zalo_tt, "source_message_id": source_message_id,
                    "as_skip_autosend": True, "skip_outbound_filter": True,
                    "as_skip_session_memory": True, "delivery_kind": "gate",
                })
            except Exception:
                pass
            try:
    ...
    async def _as_wait_thread_idle(
        ...
        *,
        ...
        timeout = (timeout_s if timeout_s is not None else
                   self._as_env_float("ZALO_WORKFLOW_TURN_TIMEOUT_S", 420.0, 30.0, 900.0))
        return await wait_thread_idle(
            ...
            pulse=pulse,
            arm_first=arm_first,
            pending_get=self._as_pending_gateway_events,
        )
    ...
    async def _as_autosend_late_files(self, thread_id: str, thread_type: str = "") -> bool:
        ...
        if not tid:
            return False
        if self._as_autosend_turn_dest(tid).get("artifact_only"):
            # Typed document jobs send the selected artifact explicitly. A
            # shared directory clock is not provenance for another request.
            return self._as_job_already_sent_file(tid)
    ...
    def _as_job_already_sent_file(self, thread_id: str) -> bool:
        binding = (getattr(self, "_as_document_jobs", None) or {}).get(str(thread_id))
        if not isinstance(binding, dict):
            try:
                from .turn_wait import real_thread_id
            except ImportError:
                from turn_wait import real_thread_id
            # Both queued direct turns and isolated workflows use Dispatcher
            # receipts. Local media markers cannot observe a separate sender.
            # Only this exact claimed turn may supply the original source;
            # never borrow the mutable global destination or another turn.
            turns = getattr(self, "_as_turns", None)
            original = turns.get(str(thread_id)) if isinstance(turns, dict) else None
            if isinstance(original, dict) and original.get("thread_id") and original.get("source_message_id"):
                binding = {**original, "thread_id": real_thread_id(original["thread_id"])}
        if isinstance(binding, dict) and binding.get("source_message_id"):
            import urllib.parse
            query = urllib.parse.urlencode({"thread_type": binding["thread_type"],
                                           "source_message_id": binding["source_message_id"]})
            result = self._as_session_http("GET", "/v1/files/receipts/" + urllib.parse.quote(binding["thread_id"], safe="") + "?" + query) or {}
            if result.get("ok") and int(result.get("acknowledged_count") or 0) > 0:
                self._as_compound_mark_delivered(thread_id)
                return True
        seen = getattr(self, "_as_job_file_sent", None)
        tid = str(thread_id or "")
            ...
            async def _run_turn() -> None:
                async with self._as_agent_turn_lock_for(tid):
                    ...
                    if isinstance(reply_quote, dict):
                        ...
                        if quote_text:
                            quote_block = f"\n\n[Quoted message]\n{quote_text}"
                            if quote_text not in prompt_text:
                                prompt_text = (
                                    f"{prompt_text.strip()}{quote_block}"
                    ...
                    try:
                        from .job_prompt import delivery_instruction
                    except ImportError:
                        from job_prompt import delivery_instruction
                    instruction = str(event.text or bare_q)
                    # Legacy text-only bridge events can lack native IDs. They
                    # may chat, but cannot acquire a fictitious artifact receipt.
                    if item.get("message_id"):
                        instruction = delivery_instruction(instruction, {
                            "thread_id": tid,
                            "thread_type": thread_type,
                            "source_message_id": str(item.get("message_id") or ""),
                        })
                    event.text = self._as_with_host_clock_context(instruction)
                    await self.handle_message(event)
    ...
    async def _on_inbound_message(self, m: Dict[str, Any]) -> None:
        ...
        if isinstance(media, dict) and media_urls:
            ...
            if raw and not attach_bare:
                try:
                    from .classify_client import classify_text_async
                except ImportError:
                    from classify_client import classify_text_async
                attachment_plan = await classify_text_async(raw,
                    thread="group" if thread_type == "group" else "dm",
                    attachments="image" if attach_is_image else "file",
                    conversation_id=str(thread_id))
                if attachment_plan.get("ok") is not False and attachment_plan.get("artifact_operation") == "convert":
                    conversion_plan = attachment_plan
                    m["plan"] = attachment_plan
            # Conversions preserve the source for the shared renderer instead
            # of running extract/OCR and replacing it with a chat excerpt.
            ...
            if conversion_plan is not None or (attach_is_image and not attach_bare):
                extract_task = None
            else:
        ...
        if media_urls:
            ...
            if _gate:
                _blocked = await _gate(
                    ...
                    artifact_operation="convert" if conversion_plan is not None else "",
                )
                if _blocked:
    ...
    def _as_autosend_turn_dest(self, chat_id=None) -> dict:  # ASSISTANT_AUTOSEND_v3
        ...
        try:
            ...
            from turn_wait import is_isolated_session
        if is_isolated_session(cid):
            hit = turns.get(cid) if isinstance(turns, dict) else None
            return hit if isinstance(hit, dict) and hit.get("source_message_id") else {}
    ...
    def _as_autosend_file_fp(self, file_path, thread_id="") -> str:
        ...
        try:
            ...
            from artifact_delivery import artifact_key
        binding = self._as_autosend_turn_dest(thread_id) or {}
        dest = self._as_zalo_api_chat_id(thread_id)
        try:
            return artifact_key(file_path, dest, binding.get("thread_type", "user"), binding.get("source_message_id", ""))
        except OSError:
            return ""

    def _as_autosend_file_claim(self, file_path, thread_id) -> bool:  # ASSISTANT_AUTOSEND_v3
        """Reserve one bound artifact; never cache a reservation as an ACK."""
        try:
            from .artifact_delivery import DeliveryClaim
        except ImportError:
            from artifact_delivery import DeliveryClaim
        key = self._as_autosend_file_fp(file_path, thread_id)
        if not key:
            return False
        seen = getattr(self, "_as_sent_fp", None)
        ...
        if key in seen:
            return False
        binding = self._as_autosend_turn_dest(thread_id) or {}
        claim = DeliveryClaim(lambda route, body: self._as_session_http("POST", route, body), file_path,
                              self._as_zalo_api_chat_id(thread_id), binding.get("thread_type", "user"),
                              binding.get("source_message_id", ""))
        try:
            data = claim.claim()
        except RuntimeError:
            logger.error("Zalo: delivery coordination unavailable")
            return False
        first = bool(data.get("first"))
        if not first and data.get("state") == "acknowledged" and data.get("message_id"):
            seen.add(key)
        if first:
            claims = getattr(self, "_as_delivery_claims", None)
            if not isinstance(claims, dict):
                claims = self._as_delivery_claims = {}
            claims[(str(thread_id), str(Path(file_path).resolve()))] = claim
        return first

    def _as_autosend_already_sent(self, file_path, thread_id="") -> bool:
        key = self._as_autosend_file_fp(file_path, thread_id)
        if not key:
            return False
        seen = getattr(self, "_as_sent_fp", None)
    ...
    def _as_autosend_file_unclaim(self, file_path, thread_id="") -> None:
        claims = getattr(self, "_as_delivery_claims", None) or {}
        key = (str(thread_id), str(Path(file_path).resolve()))
        claim = claims.get(key)
        # Outer retry handlers do not know whether a wire send was accepted.
        # Release only reservations that have never begun a wire operation.
        if claim is not None and not getattr(claim, "sending", False):
            claim.transition("release")
            claims.pop(key, None)

    def _as_delivery_begin(self, file_path, thread_id) -> bool:
        claim = (getattr(self, "_as_delivery_claims", None) or {}).get((str(thread_id), str(Path(file_path).resolve())))
        if claim is None:
            return False
        try:
            claim.transition("sending")
            claim.sending = True
            return True
        except RuntimeError:
            return False

    def _as_delivery_ack(self, file_path, thread_id, response) -> None:
        claims = getattr(self, "_as_delivery_claims", None) or {}
        key = (str(thread_id), str(Path(file_path).resolve()))
        claim = claims.get(key)
        message_id = self._as_bridge_message_id(response)
        if claim is None or not message_id:
            raise RuntimeError("attachment acknowledgement missing")
        claim.transition("acknowledged", message_id)
        self._as_sent_fp.add(claim.body["key"])
        claims.pop(key, None)

    def _as_delivery_reject(self, file_path, thread_id, response) -> None:
        if not isinstance(response, dict) or not response.get("definite_rejection"):
            return
        claims = getattr(self, "_as_delivery_claims", None) or {}
        key = (str(thread_id), str(Path(file_path).resolve()))
        claim = claims.get(key)
    ...
    async def _as_autosend_turn_files(self, chat_id, content, metadata=None):  # ASSISTANT_AUTOSEND_v5
        ...
        if dest_turn.get("artifact_only"):
            return content
        cid = str(chat_id or "")
        ...
        # A shortcut/notifier may already have delivered this turn's artifact
        # (e.g. a scheduled composed image). Never scan the shared output
        # directory again for the same turn: a stale earlier file would be
        # re-sent under the new source.
        sent_map = getattr(self, "_as_job_file_sent", None)
        if isinstance(sent_map, dict) and media_sent_in_turn(sent_map, tid, turn_token):
            return content
        ...
        for src in found:
            ...
            try:
                ...
                dest = src
            if self._as_autosend_already_sent(dest, tid):
                logger.debug("Zalo: autosend skip already-claimed %s", dest.name)
                try:
                    from .autosend import claimed_composite_is_terminal
                except ImportError:
                    from autosend import claimed_composite_is_terminal  # type: ignore
                if claimed_composite_is_terminal(str(dest)):
                    break
                continue
            try:
                dest_send = dest
                ...
                meta_send = {
                    **meta,
                    "as_skip_autosend": True,
                    "as_claimed": True,
                    "delivery_claim_path": str(dest_send),
                }
                ...
                if err == "av_blocked":
                    blocked_n += 1
                    self._as_autosend_file_unclaim(dest_send, tid)
                    continue
                if ok:
                    sent += 1
    ...
    def _as_av_required(self) -> bool:
        ...
        return raw in {"active", "1", "true", "yes", "on"}
    ...
    async def _as_av_gate(  # ASSISTANT_FILE_PIPELINE_v6
        self, thread_id, sender_id, local_path: str, media: dict, user_text: str = "", artifact_operation: str = ""
    ) -> bool:
        ...
        if not self._as_file_pipeline_enabled() and not self._as_av_required():
        ...
        if not self._as_av_activated():
            ...
            if required:
                try:
                    await self.send(chat_id=str(thread_id), content="Chưa quét được file. Vui lòng gửi lại sau.", metadata={"as_skip_inflight": True})
                except Exception:
                    pass
                return True
            ...
            if self._as_env_flag_on("AV_SCAN", "ENABLE_ANTIVIRUS", default="inactive"):
                ...
                self._as_enqueue_file_pipeline(
                    thread_id, sender_id, local_path, media, user_text=user_text, artifact_operation=artifact_operation
                )
                return False
            if required:
                try:
        ...
        import uuid
        session_id = f"zalo-{uuid.uuid4().hex}"
        t0 = time.monotonic()
        try:
            data = file_bytes
    ...
    def _as_enqueue_file_pipeline(  # ASSISTANT_FILE_PIPELINE_v6
        ...
        if artifact_operation == "convert":
            self._as_flow("file_pipeline_skip", reason="source_conversion", thread_id=thread_id)
            return
    ...
    async def send(
        ...
        try:
            ...
            from turn_wait import real_thread_id  # type: ignore
        original_binding = self._as_autosend_turn_dest(chat_id)
        try:
            from .turn_wait import is_isolated_session
        except ImportError:
            from turn_wait import is_isolated_session
        if is_isolated_session(str(chat_id)) and not original_binding:
            logger.error("Zalo: isolated outbound job has no original source binding")
            return SendResult(success=False, error="original job binding unavailable")
        metadata = dict(metadata) if isinstance(metadata, dict) else {}
        if original_binding.get("source_message_id"):
            metadata.setdefault("source_message_id", original_binding["source_message_id"])
        pending_notes = getattr(self, "_as_pending_note_persist", None) or {}
        pending_note = (
            pending_notes.get((real_thread_id(str(chat_id)), str(metadata.get("source_message_id") or "")))
            or pending_notes.get(real_thread_id(str(chat_id))) or pending_notes.get(str(chat_id))
        ...
        if original_binding.get("artifact_only") and not any(metadata.get(key) for key in (
            "is_approval_prompt", "skip_outbound_filter", "as_skip_autosend",
        )):
            # A typed document job owns its terminal outcome. It checks the
            # original receipt and emits a localized failure if delivery fails.
            # Never expose an agent's private review, before or after the ACK.
            logger.info("Zalo: defer document final text to artifact receipt outcome")
            ...
            try:
                ...
                append_turn(
                    ...
                    source_message_id=str(
                        ...
                        or self._as_active_turn_message_ids.get(str(tid))
    ...
    def _as_bridge_message_id(response) -> str:
        ...
        try:
            from .artifact_delivery import bridge_message_id
        except ImportError:
            from artifact_delivery import bridge_message_id
        return bridge_message_id(response)
    ...
    def _as_record_attachment_delivery(
        ...
        *,
        ...
        source_message_id = str(
            meta.get("source_message_id")
            or (self._as_autosend_turn_dest(chat_id) or {}).get("source_message_id")
            or self._as_active_turn_message_ids.get(str(dest_id))
            or self._as_active_turn_message_ids.get(str(chat_id))
            or self._as_source_message_id.get()
            or ""
    ...
    async def send_image_file(self, chat_id, image_path, caption=None, reply_to=None, metadata=None, **kwargs):
        ...
        claim_path = meta.get("delivery_claim_path") or image_path
        scan = getattr(self, "_as_outbound_scan", None)
        if callable(scan) and not meta.get("as_skip_outbound_av"):
            if await scan(image_path, dest_id, payload.get("fileName")) == "blocked":
                self._as_autosend_file_unclaim(claim_path, chat_id)
                return SendResult(success=False, error="av_blocked")
        if not meta.get("as_claimed") and not self._as_autosend_file_claim(claim_path, chat_id):
            acknowledged = self._as_autosend_already_sent(claim_path, chat_id)
            return SendResult(success=acknowledged, error=None if acknowledged else "delivery pending or coordination unavailable")
        if not self._as_delivery_begin(claim_path, chat_id):
            return SendResult(success=False, error="delivery coordination unavailable")
        res = await self._as_with_dest_send_lock(
            dest_id, lambda p=payload: self._post("/send-attachment", p)
        )
        if not self._as_bridge_ok(res):
            self._as_delivery_reject(claim_path, chat_id, res)
        ...
        self._as_delivery_ack(claim_path, chat_id, res)
    ...
    async def send_document(self, chat_id, file_path, caption=None, file_name=None, reply_to=None, metadata=None, **kwargs):
        ...
        if not meta.get("as_skip_outbound_av"):
            ...
            if _scan:
                ...
                if _verdict == "blocked":
                    self._as_autosend_file_unclaim(meta.get("delivery_claim_path") or file_path, chat_id)
                    return SendResult(success=False, error="av_blocked")
        claim_path = meta.get("delivery_claim_path") or file_path
        if not meta.get("as_claimed") and not self._as_autosend_file_claim(claim_path, chat_id):
            acknowledged = self._as_autosend_already_sent(claim_path, chat_id)
            return SendResult(success=acknowledged, error=None if acknowledged else "delivery pending or coordination unavailable")
        if not self._as_delivery_begin(claim_path, chat_id):
            return SendResult(success=False, error="delivery coordination unavailable")
        quote = self._sanitize_send_quote(meta.get("quote")) if isinstance(meta.get("quote"), dict) else None
        if quote:
            payload["quote"] = quote
        res = await self._as_with_dest_send_lock(
            dest_id, lambda p=payload: self._post("/send-attachment", p)
            ...
            if looks_invalid_param(err) and any(str(file_path).lower().endswith(ext) for ext in VIDEO_EXTS):
                ...
                if retry_path:
                    ...
                    if self._as_bridge_ok(res2):
                        self._as_delivery_ack(claim_path, chat_id, res2)
    ...
    async def send_video(self, chat_id, video_path, caption=None, reply_to=None, metadata=None, **kwargs):
        meta = metadata if isinstance(metadata, dict) else {}
        claim_path = meta.get("delivery_claim_path") or video_path
        if not meta.get("as_claimed") and not self._as_autosend_file_claim(claim_path, chat_id):
            return SendResult(success=False, error="delivery already acknowledged or pending")
        meta = {**meta, "as_claimed": True, "delivery_claim_path": str(claim_path)}
        ...
        if host_mp4 and host_thumb:
            ...
            if video_url and thumb_url:
                ...
                if self._as_bridge_ok(sent):
                    self._as_delivery_ack(claim_path, chat_id, sent)
                ...
                self._as_delivery_reject(claim_path, chat_id, sent)
                return SendResult(success=False, error=err)
        return await self.send_document(chat_id, staged, caption=caption, metadata=meta)
