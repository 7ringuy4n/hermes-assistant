...
def schedules_for_thread(thread_id: str, thread_type: str | None = None) -> list[dict[str, Any]]:
    """Schedules whose canonical delivery destination is this typed thread."""
    ...
    for row in list_schedules():
        ...
        origin = row.get("origin") if isinstance(row.get("origin"), dict) else {}
        context = row.get("context") if isinstance(row.get("context"), dict) else {}
        # Requester/sender identifiers are audit fields, not destinations. A
        # person's DM must not expose every group schedule they requested.
        destination = str(origin.get("thread_id") or origin.get("chat_id") or context.get("thread_id") or "").strip()
        row_type = str(context.get("thread_type") or origin.get("thread_type") or "").strip()
        if thread_type and row_type and (row_type == "group") != (thread_type == "group"):
            continue
        if tid == destination:
            out.append(row)
    return out
...
def schedules_for_record_scope(selector: dict[str, Any], *, thread_id: str,
                               thread_type: str, is_admin: bool) -> list[dict[str, Any]]:
    try:
        from .record_scope import resolve_record_target
    except ImportError:
        from record_scope import resolve_record_target
    target, kind = resolve_record_target(selector, thread_id=thread_id,
                                         thread_type=thread_type, is_admin=is_admin)
    return list_schedules() if target is None else schedules_for_thread(target, kind)
