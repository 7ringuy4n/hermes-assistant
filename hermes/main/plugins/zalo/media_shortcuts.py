...
_COMPOSITION_PLAN_MAX_TOKENS = 8192
...
def _omni_json_plan_once(
    base: str, key: str, model: str, system: str, user: str, *, max_tokens: int
) -> tuple[dict[str, Any] | None, str, str]:
    """One structured planning attempt; returns (parsed, text, finish_reason)."""
    body = json.dumps(
        {
            "model": model,
            "stream": False,
            ...
            # Structured JSON planning must not spend the completion budget on
            # extended reasoning; a reasoning model otherwise truncates at
            # finish_reason=length and the plan is lost.
    ...
    return _json_object(text), text, finish


def _omni_json_plan(system: str, user: str, *, max_tokens: int) -> dict[str, Any]:
    """Run one prompt-asset-owned structured planning call.

    Retries once with a bounded larger budget when a reasoning model truncates
    the response (``finish_reason=length``); other failures are not retried.
    """
    try:
    ...
    for attempt in range(2):
        parsed, text, finish = _omni_json_plan_once(
            base, key, model, system, user, max_tokens=budget
        )
        if parsed:
            return parsed
        last_text, last_finish = text, finish
        if finish != "length" or attempt == 1:
            break
...
def _synthesize_composition_plan(
    ...
    *,
    ...
    if not facts and not panels:
        ...
        return {}
    # A shared-region plan may echo the same rows at the top level and inside a
    # panel. Rendering both draws the informational copy twice; keep the panel
    # copy and drop the duplicated top-level rows (distinct rows are preserved).
    if panels:
        panel_keys = {
            f"{row['label'].casefold()}:{row['value'].casefold()}"
            for panel in panels
        ...
        facts = [
            row
            for row in facts
            if f"{row['label'].casefold()}:{row['value'].casefold()}" not in panel_keys
        ]
    return {
        "title": title,
        "facts": facts,
