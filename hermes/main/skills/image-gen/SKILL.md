...
In a direct Hermes CLI/API session there is no Zalo host to execute a scenic
plan. Generate through the same Router Worker image capability, or Dispatcher
`/v1/scenic-still` for an internal document asset, without a Zalo destination.
Return the image through the direct session's native artifact mechanism. Do
not activate Zalo or Message Worker, fabricate a conversation identity, or
claim an autosender delivered it. Generation requires its configured capability
service, not a messaging integration. All composition and copy rules below
apply equally to direct and messaging sessions.
