...
| **Schedule** | `schedule` | Go schedule worker (Postgres; SQLite only without DSN). Hermes does not tick cron. |
| **Notes** | `notes` | Memory Worker scoped CRUD/list/detail/selection; direct use without messaging |
| **Conversion** | `file-convert` | Shared Media worker; native print/raster or explicit editable content reflow; quoted source priority |
