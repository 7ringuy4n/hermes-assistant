...
Direct Hermes sessions use native artifact delivery, not a Zalo autosender or
Message Worker. Office generation sets `send_zalo=false` and needs no chat ID;
images use the same image capability without a Zalo send. Return the completed
artifact once through the session's native file/link mechanism. Do not start a
messaging integration merely to satisfy direct-session delivery.
...
| Office | `file-gen` → private `POST /v1/office-file`, native-text/page-image review, then one selected-file delivery; not Hermes pdf skill |
