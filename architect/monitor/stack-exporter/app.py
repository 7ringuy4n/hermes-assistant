...
SCHEDULE_URL = os.environ.get("SCHEDULE_URL", "http://schedule-worker:8110").rstrip("/")
WORKFLOW_QUEUE_KEY = os.environ.get("WORKFLOW_QUEUE_KEY", "wf:queue")
ZALO_GATE_QUEUE_PATTERN = os.environ.get("ZALO_GATE_QUEUE_PATTERN", "assistant:gate:q:*")
...
def _redis_scan_keys(host: str, port: int, pattern: str, db: int = 0) -> list[str]:
    try:
        _redis_cmd(host, port, "SELECT", str(db))
    except OSError:
        return []
...
def _redis_llen(host: str, port: int, key: str) -> float:
    try:
        raw = _redis_cmd(host, port, "LLEN", key)
        text = raw.decode("utf-8", errors="replace").strip()
        if text.startswith(":"):
            return float(text[1:].split("\r")[0])
    except Exception:
        pass
    return 0.0
...
def _schedule_lag_seconds() -> float:
    """Seconds the earliest enabled schedule is overdue (0 when none are due)."""
    try:
        data = _http_json(f"{SCHEDULE_URL}/v1/schedules", timeout=5.0)
    except Exception:
        return 0.0
    rows = data if isinstance(data, list) else (data.get("schedules") or data.get("items") or [])
    now = time.time()
    lag = 0.0
    for row in rows:
...
def render_metrics() -> str:
    lines: list[str] = [
        ...
        "# HELP assistant_qdrant_query_seconds Qdrant collection query latency",
        "# TYPE assistant_qdrant_query_seconds gauge",
        "# HELP assistant_queue_depth Pending items in a service queue",
        "# TYPE assistant_queue_depth gauge",
        "# HELP assistant_schedule_lag_seconds Seconds the earliest enabled schedule is overdue",
        "# TYPE assistant_schedule_lag_seconds gauge",
    ]
    now = time.time()
    try:
        ...
        # Queue depth: ingest jobs, workflow jobs, and per-thread Zalo gate queues.
        ingest_depth = _redis_llen(host, port, "ingest:jobs")
        lines.append(_line("assistant_redis_ingest_queue_len", ingest_depth))
        lines.append(_line("assistant_queue_depth", ingest_depth, {"queue": "ingest"}))
        lines.append(
            _line(
                "assistant_queue_depth",
                _redis_llen(host, port, WORKFLOW_QUEUE_KEY),
                {"queue": "workflow"},
            )
        )
        try:
            zalo_keys = _redis_scan_keys(host, port, ZALO_GATE_QUEUE_PATTERN, db)
            zalo_depth = sum(_redis_llen(host, port, k) for k in zalo_keys)
        except Exception:
            zalo_depth = 0.0
        lines.append(_line("assistant_queue_depth", zalo_depth, {"queue": "zalo"}))
    ...
    for coll in (COLL_KNOWLEDGE, COLL_MEMORY):
        ...
        lines.append(_line("assistant_qdrant_query_seconds", time.time() - t0, labels))
    ...
    lines.append(_line("assistant_schedule_lag_seconds", _schedule_lag_seconds()))
