    ...
    try:
        ...
        with httpx.Client(timeout=10) as c:
            if c.get(f"{AV_URL}/health").status_code != 200:
                return {"ok": False, "reason": "av_down"}
            ...
            while time.monotonic() < deadline:
                ...
                if st.status_code != 200:
                    return {"ok": False, "reason": "av_error"}
                state = st.json()
                infected = int(state.get("infected") or 0)
                if infected > 0 or state.get("blocked") is True:
                    return {"ok": False, "reason": "infected" if infected > 0 else "av_error"}
                if (state.get("ready") is True and state.get("status") == "READY_FOR_PROCESSING"
                        and int(state.get("scanning") or 0) == 0 and int(state.get("clean") or 0) > 0):
                    return {"ok": True, "detail": state}
                time.sleep(min(0.25, max(0.0, deadline - time.monotonic())))
            return {"ok": False, "reason": "av_timeout"}
    except Exception:
        # Enabled antivirus is an isolation boundary, not an optional heuristic.
        # Its failure must never become CLEAN even with generic fail-open policy.
        return {"ok": False, "reason": "av_exception"}
