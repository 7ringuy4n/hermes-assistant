"""assistant Memory Worker — Postgres SoT, optional Qdrant index.
...
def _timing_add(field: str, seconds: float, thread_id: Optional[str] = None) -> None:
    ...
    if v not in {"1", "true", "yes", "on", "active"} or seconds < 0.001:
        return
    try:
...
def query_notes(req: NoteQueryReq) -> dict[str, Any]:
    ...
    if req.scope_ids and any(not value.startswith(("zalo:group:", "zalo:user:"))
                             or len(value) > 256 for value in req.scope_ids):
        raise HTTPException(422, "invalid conversation scope")
    clauses = ["scope_id=ANY(%s)" if req.scope_ids else "scope_id=%s", "active"]
    params: list[Any] = [req.scope_ids if req.scope_ids else req.scope_id]
    if req.id:
        clauses.append("id=%s")
...
def assemble_context(req: ContextReq) -> dict[str, Any]:
    ...
    try:
        ...
        system_hints = [
            ...
            "Do not dump docs into MEMORY.md; call Memory Worker for durable facts.",
...
def compact(limit: int = Query(50, ge=1, le=200)) -> dict[str, Any]:
    ...
    if not EMBED_URL or not QDRANT_URL:
        return {
            "ok": not bool(embed_err),
            "reindexed": 0,
            "deactivated_staged": deactivated,
            "embed_model": model,
            "embed": False,
            "note": "embedding or vector index inactive",
        }
    for row in rows:
        try:
            indexed = _index_memory(
                str(row["id"]),
...
def _index_memory(mid: str, content: str, typ: str, importance: float) -> bool:
    ...
    httpx.put(
        ...
        json={
            "points": [
                {
                    "id": str(uuid.uuid5(uuid.NAMESPACE_URL, "hermes-stack:memory:" + mid)),
                    "vector": vec,
                    "payload": {
                        "memory_id": mid,
                        "type": typ,
