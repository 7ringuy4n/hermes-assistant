...
| `POST /v1/file-convert` | Convert approved existing files; native print/raster or explicit editable content reflow |
| `POST /v1/text-poster` | Render exact requested glyphs deterministically |
| `POST /v1/office-file` | Create txt/csv/md/xlsx/docx/**pdf**/pptx when `OFFICE_FILE_GEN=active`; formatted authoring requires `draft=true, send_zalo=false`, returns private native-text/page-image review, then explicitly deliver the selected artifact |
