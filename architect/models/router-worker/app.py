...
@app.middleware("http")
async def _record_llm_metrics(request: Request, call_next):
    """Record LLM routing signals from the response provider/task headers."""
    start = time.time()
    response = await call_next(request)
    try:
        provider = response.headers.get("x-router-worker-provider")
        if provider:
            capability = response.headers.get("x-router-worker-task") or "normal"
            status = "ok" if response.status_code < 500 else "error"
            metrics_mod.observe(
                capability=capability,
                provider=provider,
                model=response.headers.get("x-router-worker-model") or "",
                status=status,
                seconds=time.time() - start,
            )
            if status == "ok" and provider != "omni-router":
                metrics_mod.fallback(from_provider="omni-router", to_provider=provider)
    except Exception:
        pass
    return response
...
async def classify_endpoint(request: Request) -> dict[str, Any]:
    ...
    for _name, base, headers, model in candidates:
        ...
        if _ok:
            if _name != "omni-router":
                metrics_mod.fallback(from_provider="omni-router", to_provider=_name)
            return last
...
async def outbound_endpoint(request: Request) -> dict[str, Any]:
    ...
    for _name, base, headers, model in candidates:
        ...
        _ok = bool(last.get("ok")) and str(last.get("action") or "") in {"send", "drop"}
        metrics_mod.observe(
            capability="outbound",
            provider=_name,
            model=str(model or ""),
