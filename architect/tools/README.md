...
| Chat LTM | Memory Worker / Postgres | User preferences / facts |
...
**Auto-learn @ 00:00:** promote eligible files into Qdrant without admin approve only while the switch is off (`LEARN_REQUIRE_APPROVE` unset/`0`/`inactive`); while on (`active|1|true|yes|on`) scan ingest is staged pending.
