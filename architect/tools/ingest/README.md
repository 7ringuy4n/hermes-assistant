...
`REDIS_URL` (Valkey; env name kept for RQ compatibility), `QDRANT_URL`, `EMBED_URL`, `LEARN_LIST_LIMIT`, `LEARN_REQUIRE_APPROVE` (on = `active|1|true|yes|on`; pending approval is required for scan/midnight ingest while on, off = `0`/`inactive` auto-ingests; Zalo file submit is always pending), `LEARN_NOTIFY_PATH`, `NOTIFY_URL`, `ZALO_BRIDGE_URL`, `ZALO_ADMIN_USERS_FILE`, media roots under `/data/assistant`.
