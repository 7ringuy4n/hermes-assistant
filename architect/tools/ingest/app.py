...
# Shared lab job bus (also used by memory-worker index jobs)
...
# Standard stack switch semantics: only an explicit on-value enables approval.
# Writing "inactive" (the stack's off convention) must NOT enable approval.
LEARN_REQUIRE_APPROVE = (os.environ.get("LEARN_REQUIRE_APPROVE") or "0").strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
