## 2026-09-15 +07 — safe note mutations and composed-image delivery

Bulk destructive note mutations preview exact matched ids and require explicit
confirmation before any delete/update; delete/update report their own outcome.
Zalo plugin vendors `artifact_delivery.py`; read-only plugin source mount had
shadowed the runtime copy, breaking composed-image delivery (`ModuleNotFoundError`).
See [`history/2026-09-15/README.md`](../history/2026-09-15/README.md).

## 2026-09-14 16:21 +07 — merge override and honest observer coverage

User authorizes known-failure develop/main promotion. VPS15/26 PASS,11unpassed;
latest PDF240s FAIL, greeting10,898ms vs5,000ms, native-client quote open.
Latest full offline134/134 PASS predates final observer refinements. Negated
attribution oracle focused checks and conversion/burst embedded syntax pass;
fresh conversion/burst live runs are not claimed. Private reports stay local.

## 2026-09-14 15:54 +07 — native quote concurrency PASS

Gate19 passes in48.89s: real cached CLI identities, native quoted DM/group ACKs,
exact answers, no crossover and terminal settlement. Inputs remain injected,
not real-client file quotes. VPS14/26 passed;12unpassed. Latest attribution
skill refinement validates and is normally deployed; PDF/offline reruns running.
Actual-storage/date/title/cleanup note-locale observer staged; live run pending.

## 2026-09-14 15:51 +07 — source-bound refusal and native quote observer

Gate 14 passes exact source native refusal, terminality, no attachments/late
extras and semantic 10/10. Correct its first oracle timeout: host policy
results are intentionally tagged `gate`, not excluded control noise. Progress
13/26;13 unpassed. Grounding candidate offline134/134 passes; PDF still fails
240s and source interpretation. Latest staged refinement unverified. Gate 19
now requires native cached CLI identity/quoted ACK/exact reply/terminality;
injected contexts do not certify client-input quote bubbles. Rerun running.

## 2026-09-14 15:42 +07 — isolated environment and OpenBao gates

Gates 17/26 (three checks) and 13/26 (four checks) pass with real shared
cleanup/load/scrub scripts, private fixtures, isolated CAS-versioned KV and a
network-disabled temporary consumer. Exact owned cleanup and unchanged live
KV/token/files/consumer env verified. Do not run the retired invalid-key
production rotation or world-writable fixture harness. Fresh progress 12/26;
14 remain unpassed. PDF gate fails timeout and native factual review; source
legend regression/skill update normally deployed, fresh reruns pending.

## 2026-09-14 15:09 +07 — isolated compact and original-source notes

Fresh deferred-note deployment passes 134/134 offline entries and gate 24/26:
source-provenanced notes, requested identifier, terminal silence, owned cleanup.
Progress: 10/26 passed, 16 unpassed. PDF gate observer is strengthened before
its pending live rerun; no draft/log-only or quota-skipped certification.

Gate 15/26 passes real embeddings and isolated SQL/vector production-route
compaction with repeat grounding and owned cleanup. The memory fix passes
134/134 offline entries. A subsequent reproduced deferred-note source-isolation
and scope/provenance fix is normally deployed, with a validated notes prompt.
Its full offline and gate 24/26 reruns are running. Nine VPS gates pass; 17
remain unpassed. Prior marker-omitting fixture notes are backed up and exactly
soft-deleted; latency and native client quote delivery are not certified passed.

## 2026-09-14 14:58 +07 — fresh verification and vector-index follow-up

Gate 20/26 passes original-source concurrency, semantic 9/10 and visual review
of both exact acknowledged DOCX pages. Eight VPS gates pass; 18 remain unpassed.
Gate 5/26 fails its unchanged 5,000 ms SLO at 10,898 ms; gate 24/26 is running.
A newly reproduced vector-index/compact bug is fixed in source with five focused
passing regressions; normal deployment and fresh offline/live verification
remain required.

## 2026-09-14 — scoped records, standalone capabilities, and document integrity

Later clean concurrency exposes a silent private shortcut draft containing
creation instructions. Path precedence and model-owned natural document routing
are fixed; two focused suites pass. Conversion HTTP-boundary original-source
loss is also fixed with four passing cases and a validated narrow skill update.
The newest source requires normal rebuild/recreation and fresh offline/live
reruns; earlier 134/134 evidence predates these later fixes.

Latest normally deployed candidate passes 134/134 offline index entries,
terminal antivirus gate 9/26 (5/5), and a durable-source greeting gate 3/26.
The greeting harness no longer accepts response-ready logs as delivery proof.
Seven VPS gates are freshly passed; 19 plus native quoted-file delivery remain
unpassed. Clean DM/group capability concurrency is running again.

Follow-up on operator-deployed main: fix direct queued artifact receipt lookup,
supply the claimed delivery envelope to normal queued execution, and reject
sourceless messaging sends. See the production-gap audit for the reproduced
group-type failure and pending live rerun; no new conditional promotion yet.

Continued activation inventory: optional bundles already active; normal
antivirus/monitor install backed up and verified before canonical flag changes.
Fix canonical-active lifecycle removal and wire required antivirus to both
senders. Live clean/EICAR/pre-parse rejection checks and twelve native conversion
cases pass. Full fresh release gates remain open. Architecture inventory:
`docs/07-worker-feature-catalog.md` (all 41 Compose services checked, links valid).

See `history/2026-09-14/README.md` for core fixes, the Memory Worker rename,
initial failed document gates, preserved lab configuration, and pending reruns.
The candidate adds file-convert and native asynchronous standalone schedule
admission. Conversion engine/core scope/direct-note cases pass; fresh document
content quality failed in the initial runs. Later source-bound/result-only fixes
pass fresh PDF/PPTX single-file delivery and every native page/slide review;
eight conversational conversion cases also pass. Native quote-bubble and broader
release gates remain open, so no conditional merge is performed.

Normal backup+verify and destroy→up retain data volumes. Post-clean health,
combo membership and monitoring gates pass; required AV remains active and
Zalo returns with one consumer. The large-corpus recall lab uses unique owned
tables, exact scoped needles and fail-closed cleanup. Ten million rows return
indexed exact results within the configured budget and no fixture table remains.
Concurrency harnesses must not accept filtered attachment counts before the
gateway session is terminal; use all-row counts and current replica leases.
...
# 2026-09-14 13:38 +07 — operator requests promotion despite incomplete live gate

See `history/2026-09-14/production-gap-audit.md`: explicit one-time merge
authorization preserves the concurrency failure and outstanding verification;
no automatic VPS update is included.
