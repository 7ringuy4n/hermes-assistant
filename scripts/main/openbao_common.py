...
# Operational retention values are configuration, not secrets: they live in the
# host environment (`.env` / `docs/config/DEFAULTS.md`, default 7) so an operator
# can change the rotation window without editing the secret store. These names
# are purged from existing OpenBao KV so a single source of truth remains.
RETIRED_RUNTIME_KEYS = (
    "BACKUP_RETENTION_DAYS",
    "MEMORY_STAGED_RETENTION_DAYS",
)
