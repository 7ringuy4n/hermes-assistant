...
_install_pairs() {
  ...
  case "$1" in
    ...
    message|zalo)
      ...
      # A messaging channel has an admin who can approve: knowledge-learn asks
      # from a channel user must wait for admin approval.
      echo LEARN_REQUIRE_APPROVE=active
      ;;
    monitor|mon)
...
_uninstall_pairs() {
  case "$1" in
    ...
    message|zalo)
      ...
      # No messaging channel / admin: the default Hermes operator may learn
      # without an approval step.
      echo LEARN_REQUIRE_APPROVE=inactive
      ;;
    monitor|mon|grafana)
