...
Hermes loads skills from the compose bind mount; **post-ready-learn** mirrors markdown into `$ASSISTANT_DATA_DIR/docs/` and runs ingest `learn/scan` (auto only when `LEARN_REQUIRE_APPROVE` is off: unset/`0`/`inactive`; staged pending when on).
