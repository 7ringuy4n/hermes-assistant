...
from zalo_native_quote_lab import native_quote
from scoped_documents_smoke_lab import delivered as source_delivered,thread_terminal
...
def seed(thread_id,thread_type):
    ...
    return native_quote(value,"The city established for this conversation is Hue.",own)
...
while time.time()<deadline:
    ...
    for thread_id,thread_type in destinations:
        ...
        if len(history_rows(thread_id,thread_type,"processing"))<4:
            complete=False
        if not thread_terminal(thread_id):
            complete=False
    if complete:
...
for thread_id,thread_type in destinations:
    ...
    for index,source_id in enumerate(request_ids):
        rows=source_delivered(source_id)
        if len(rows)!=1 or not str(rows[0].get('message_id') or '').isdigit():
            raise SystemExit('FAIL_SOURCE_NATIVE_ACK_OR_EXTRAS')
        if (rows[0].get('meta') or {{}}).get('file_name'):
            raise SystemExit('FAIL_UNREQUESTED_ARTIFACT')
        if index==1 and (rows[0].get('meta') or {{}}).get('quoted') is not True:
            raise SystemExit('FAIL_NATIVE_OUTBOUND_QUOTE')
...
for thread_id,thread_type in destinations:
    if not thread_terminal(thread_id):
        raise SystemExit('FAIL_LATE_ACTIVE_AGENT')
    for source_id,content in delivered[thread_type]:
        rows=source_delivered(source_id)
        if len(rows)!=1 or rows[0].get('content')!=content:
            raise SystemExit('FAIL_LATE_EXTRA_DELIVERY')
...
print(json.dumps({{
    ...
    "incoming_quote_context":"injected","native_client_quote_input_verified":False,
