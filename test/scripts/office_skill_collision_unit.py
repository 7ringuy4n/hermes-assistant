...
FORBIDDEN = {"pdf", "docx", "xlsx", "powerpoint", "pptx"}
...
def main() -> int:
    ...
    if ".curator_suppressed" not in sh or 'for _n in pdf docx xlsx' not in sh:
        ...
        return 1
    if "for _n in pdf docx xlsx powerpoint pptx" not in sh:
        print("FAIL presentation backfill must be suppressed", file=sys.stderr)
        return 1
    presentation = (SKILLS / "powerpoint/SKILL.md").read_text(encoding="utf-8")
    if "file-gen" not in presentation or "output_type=pptx" not in presentation:
        print("FAIL presentation wrapper must route to shared renderer", file=sys.stderr)
        return 1
