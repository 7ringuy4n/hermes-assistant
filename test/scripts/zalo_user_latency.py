...
with psycopg.connect(os.environ["DATABASE_URL"]) as c:
 row=c.execute("SELECT count(*),extract(epoch FROM min(created_at))::double precision FROM zalo_message_history WHERE event='delivered' AND thread_id=%s AND thread_type='user' AND meta->>'source_message_id'=%s AND coalesce(meta->>'delivery_kind','result')='result'",(os.environ["LAB_THREAD"],os.environ["LAB_SOURCE"])).fetchone()
 print(json.dumps([int(row[0]),row[1]]))
'''
    containers = subprocess.check_output(["docker","ps","--filter","label=com.docker.compose.service=zalo-api","--format","{{{{.Names}}}}"],text=True).splitlines()
    if len(containers) != 1:
        raise SystemExit("FAIL_LATENCY_AUDIT_UNAVAILABLE")
    ack = json.loads(subprocess.check_output(["docker","exec","-e","LAB_THREAD="+uid,"-e","LAB_SOURCE="+tag,containers[0],"python3","-c",probe],text=True))
    if ack[0] > 1:
        raise SystemExit("FAIL_EXTRA_LATENCY_DELIVERY")
    if ack[0] == 1:
        send_ms = int((float(ack[1]) - t0) * 1000)
        print("DURABLE_SEND_ACK_MS", send_ms)
        break
...
# Unrelated provider/failover logs cannot waive an end-to-end latency SLO.
# A proven source-correlated external block is recorded separately, never PASS.
if not ok_slo:
    raise SystemExit(3)
PY
"""
