...
def wait_zalo_delivery(
    ...
    *,
    ...
    the release oracle used by test/README.md.
    """
...
def main() -> int:
    ...
    payload = json.dumps({"checks": checks, "ok": ok}, ensure_ascii=False, indent=2)
    # Atomic replace: a prior sudo-backed run can leave this path root-owned, so
    # never open the existing inode for truncation.
    report_path = Path("/tmp/hs-suite-report.json")
    with tempfile.NamedTemporaryFile(
        "w", encoding="utf-8", dir="/tmp", prefix="hs-suite-report.", suffix=".tmp", delete=False
    ) as handle:
