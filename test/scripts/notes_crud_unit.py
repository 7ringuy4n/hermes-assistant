...
def main() -> int:
    ...
    with patch.object(notes_client, "_request", side_effect=fake_request):
        ...
        print("running test case 6/10 date-range bulk update requires confirmation")
        ...
        assert bulk_update.get("success") is False and bulk_update.get("error") == "confirm"
        assert bulk_update.get("count") == 3 and len(bulk_update.get("ids") or []) == 3
        assert all(note_id in STORE for note_id in bulk_update["ids"])
        confirmed_update = notes_client.execute_note_plan(
            {
                "skill_action": "update",
                "confirmed": True,
                "note_selector": {"date_from": "2026-09-11", "date_to": "2026-09-11", "bulk": True},
            ...
            thread_id="u1", thread_type="user", sender_id="u1",
            override_ids=list(bulk_update["ids"]),
        )
        assert confirmed_update["success"] and confirmed_update["count"] == 3
        ...
        print("running test case 8/10 delete all requires confirmation")
        deleted_all = notes_client.execute_note_plan(
            {"skill_action": "delete", "note_selector": {"match_all": True, "bulk": True}},
            thread_id="u1", thread_type="user", sender_id="u1",
        )
        assert deleted_all.get("error") == "confirm" and deleted_all.get("count") == 2
        assert len(STORE) == 2
        confirmed_delete = notes_client.execute_note_plan(
            {"skill_action": "delete", "confirmed": True, "note_selector": {"match_all": True, "bulk": True}},
            thread_id="u1", thread_type="user", sender_id="u1",
            override_ids=list(deleted_all["ids"]),
        )
        assert confirmed_delete["success"] and confirmed_delete["count"] == 2 and not STORE
