...
def main() -> int:
    ...
    # Retention moved out of OpenBao into host config: it is no longer seeded,
    # no longer scrubbed from .env, and is purged from existing KV.
    assert "BACKUP_RETENTION_DAYS" in RETIRED_RUNTIME_KEYS
    assert "MEMORY_STAGED_RETENTION_DAYS" in RETIRED_RUNTIME_KEYS
    assert "BACKUP_RETENTION_DAYS" not in ENV_SCRUB_KEYS
    assert "MEMORY_STAGED_RETENTION_DAYS" not in ENV_SCRUB_KEYS
    env_example = (ROOT / ".env.example").read_text(encoding="utf-8")
    defaults_md = (ROOT / "docs" / "config" / "DEFAULTS.md").read_text(encoding="utf-8")
    for key in ("BACKUP_RETENTION_DAYS", "MEMORY_STAGED_RETENTION_DAYS"):
        assert f"{key}=7" in env_example, key
        assert key in defaults_md, key
    seed_script = (ROOT / "scripts" / "main" / "first-setup-openbao.py").read_text(
        encoding="utf-8"
    )
    assert "RETIRED_RUNTIME_KEYS" in seed_script
    assert "RUNTIME_DEFAULTS" not in seed_script
    ...
    # Obsolete purge semantics (secret aliases + retired runtime retention)
    data = {
        "OMNIROUTER_API_KEY": "x",
        "FAL_KEY": "gone",
        "BACKUP_RETENTION_DAYS": "7",
        "MEMORY_STAGED_RETENTION_DAYS": "7",
    }
    for k in (*OBSOLETE_SECRET_KEYS, *RETIRED_RUNTIME_KEYS):
        data.pop(k, None)
    assert "FAL_KEY" not in data
    assert "BACKUP_RETENTION_DAYS" not in data
    assert "MEMORY_STAGED_RETENTION_DAYS" not in data
