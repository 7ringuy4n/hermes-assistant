...
def test_document_instructions_are_not_rendered_as_document_content() -> None:
    plan = {"ok": True, "task_hint": "file", "task_type": "file_processing",
            "output_type": "docx", "process_original_message": False,
            "instructions": ["Create a DOCX named fixture.docx with title Fixture and body marker MARKER."]}
    assert plan_allows_office_shortcut(plan) is False, "operational request reached literal renderer"
    assert plan_media_shortcut_gate(plan) == "", "file authoring must reach the model-owned skill"


def test_shortcut_uses_renderer_path_not_display_filename() -> None:
    import ast
    ...
    method = next(n for n in ast.walk(tree) if isinstance(n, ast.AsyncFunctionDef)
                  and n.name == "_as_run_host_media_shortcut")
    assignment = next(n for n in ast.walk(method) if isinstance(n, ast.Assign)
                      and any(isinstance(t, ast.Name) and t.id == "artifact_path" for t in n.targets)
                      and isinstance(n.value, ast.Call))
    with tempfile.TemporaryDirectory() as temp:
        path = Path(temp) / "private.docx"
        path.write_bytes(b"private fixture")
        namespace = {"shortcut": {"file": path.name, "path": "/worker/private.docx",
                                   "hermes_path": str(path)}}
        exec(compile(ast.Module(body=[assignment], type_ignores=[]), "actual shortcut path", "exec"), namespace)
        assert namespace["artifact_path"] == str(path), "display filename hid exact private artifact"
        assert Path(namespace["artifact_path"]).is_file()
...
def main() -> int:
    test_shortcut_uses_renderer_path_not_display_filename()
    test_document_instructions_are_not_rendered_as_document_content()
    ...
    shared_scan_guard = adapter_source.index(
        "if not artifact_delivered:", direct_send
    )
    ...
    failure_branch = adapter_source[shared_scan_guard:adapter_source.index(
        "from .session_memory import append_turn", shared_scan_guard)]
    assert "self._as_gate_announce(" in failure_branch
    assert "self._as_autosend_late_files(" not in failure_branch
    # Display names are part of the bound artifact identity, not legacy
    # heuristics that strip arbitrary "send-" prefixes from user files.
    sys.path.insert(0, str(ROOT / "architect/lib"))
    from artifact_delivery import artifact_key
    with tempfile.TemporaryDirectory() as temp:
        plain = Path(temp) / "report.pdf"
        prefixed = Path(temp) / "send-report.pdf"
        plain.write_bytes(b"same fixture bytes")
        prefixed.write_bytes(plain.read_bytes())
        assert artifact_key(plain, "fixture", "user", "source") != artifact_key(prefixed, "fixture", "user", "source")
