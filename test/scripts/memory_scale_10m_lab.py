...
PostgreSQL predicates/index families as Memory Worker, and removed in finally.
...
TABLE = "memory_scale_lab_" + uuid.uuid4().hex
...
try:
    ...
    psql(f"""
      ...
      SELECT n,
             CASE WHEN n = 1700003 OR n % 5 = 0 THEN 'task' ELSE 'fact' END,
             CASE
               WHEN n = 824731 THEN 'harborquartz knowledge anchor exact result'
               WHEN n IN (1700003, 1700004) THEN 'oldsession task cobalt itinerary exact result'
    ...
    knowledge_sql = f"""
      ...
      WHERE active AND NOT staged AND type='fact'
    ...
    note("knowledge_accuracy", knowledge == "824731|harborquartz knowledge anchor exact result", knowledge)
    ...
    session_sql = f"""
      SELECT id, content FROM {TABLE}
      WHERE active AND NOT staged AND type='task'
        AND (thread_id='thread-target' OR thread_id IS NULL)
    ...
    note("old_session_accuracy", session == "1700003|oldsession task cobalt itinerary exact result", session)
    ...
    try:
        ...
        raise RuntimeError("memory_scale_fixture_cleanup_failed") from exc
PY
'''
    try:
