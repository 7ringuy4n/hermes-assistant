...
while time.time() < deadline:
    # A response-ready log is not a send ACK. Use the exact original source
    # and durable transport event even when INFO logs are disabled.
    probe = '''
import os,psycopg
with psycopg.connect(os.environ["DATABASE_URL"]) as conn:
 row=conn.execute("SELECT count(*) FROM zalo_message_history WHERE event='delivered' AND thread_id=%s AND thread_type='user' AND meta->>'source_message_id'=%s AND coalesce(meta->>'delivery_kind','result')='result'",(os.environ["LAB_THREAD"],os.environ["LAB_SOURCE"])).fetchone()
 print(int(row[0]))
'''
    api_containers = subprocess.check_output(["docker","ps","--filter","label=com.docker.compose.service=zalo-api","--format","{{{{.Names}}}}"],text=True).splitlines()
    if len(api_containers) != 1:
        raise SystemExit("FAIL_ZALO_API_RESOLUTION")
    ack = subprocess.check_output(["docker","exec","-e","LAB_THREAD="+uid,"-e","LAB_SOURCE="+tag,api_containers[0],"python3","-c",probe],text=True).strip()
    if ack == "1":
        inbound_ms = inbound_ms or int((time.time()-t0)*1000)
        send_ms = int((time.time()-t0)*1000)
        print("DURABLE_SEND_ACK_MS", send_ms)
        break
    if ack != "0":
        raise SystemExit("FAIL_EXTRA_GREETING_DELIVERY")
