...
sys.path.insert(0, "/opt/assistant/architect/lib")
...
with psycopg.connect(os.environ["DATABASE_URL"]) as conn:
    rows=conn.execute(
        ...
        "coalesce(meta->>'attachment_kind',''),coalesce(meta->>'file_name',''),created_at,message_id "
...
def wait_deliveries(expected,started,kind,deadline_seconds):
    ...
    while time.time()<deadline:
        ...
        for label,row in expected.items():
            ...
            all_rows=delivery_rows(thread_id,thread_type,source_id,started)
            if len(all_rows)>1:
                # An extra acknowledged result cannot disappear as the job
                # settles. Fail immediately instead of hiding it in a filter
                # or waiting the full operation budget for an impossible PASS.
                raise SystemExit("FAIL_EXTRA_DELIVERY_"+kind.upper()+"_"+label.upper())
            rows=all_rows
            if kind=="document":
            ...
            found[label]=rows
            if len(rows)!=1 or len(all_rows)!=1:
                complete=False
            elif label not in latencies:
                latencies[label]=round(time.time()-started,2)
        complete=complete and all(thread_terminal(row[0]) for row in expected.values())
        if complete:
            ...
            settled_since=None
        time.sleep(3)
    raise SystemExit("FAIL_TERMINAL_DELIVERY_"+kind.upper())
...
"""
    raw=subprocess.check_output(["docker","exec","-e","LAB_KEYS="+json.dumps(list(keyed)),
        ...
        "-e","LAB_MESSAGE="+str(message_id),service_container("session"),"python3","-c",probe],text=True)
    selected=json.loads(raw)
    # Identical private copies have the same bytes/name/source identity. Only
    # the content-bound wire ACK can select the delivered document; never mtime.
    if len(selected)!=1:
        raise SystemExit("FAIL_FILE_ACK_IDENTITY")
...
file_rows,file_latencies=wait_deliveries(file_expected,file_started,"document",900)
...
for label,rows in file_rows.items():
    ...
    root_path=pathlib.Path("/data/assistant/media/out").resolve()
    candidates=[path for path in root_path.rglob(file_name)
        if path.is_file() and path.stat().st_mtime>=file_started
        and path.resolve().is_relative_to(root_path)]
    path=acknowledged_artifact(candidates,file_expected[label],rows[0][5])
...
print(json.dumps({{
    ...
    "files":{{
        ...
        "packages_valid":True,
        "content_exact":True,
        "artifact_receipts_verified":True,
...
'''
    if (os.environ.get("ASSISTANT_TEST_LOCAL") or "").strip().casefold() in {"1", "true", "yes"}:
        ...
        client = connect()
        try:
            raw = sanitize(sudo_bash(client, remote, timeout=1500) or "")
        finally:
            client.close()
