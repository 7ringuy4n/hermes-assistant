#!/usr/bin/env python3
"""Live Zalo lab: injected quote contexts, native quoted DM/group output.
...
cd /opt/assistant
python3 - <<'PY'
import concurrent.futures, json, os, pathlib, pwd, re, subprocess, time, urllib.parse, urllib.request
import sys
sys.path.insert(0,'/opt/assistant/test/scripts')
from scoped_documents_smoke_lab import thread_terminal
...
def native_quote(message_id, seed):
    # Resolve the current listener, not archived state or a fabricated cli id.
    sockets=subprocess.check_output(['ss','-ltnp','( sport = :8787 )'],text=True)
    pids=set(re.findall(r'pid=(\d+)',sockets))
    if len(pids)!=1:
        raise SystemExit('FAIL_CURRENT_BRIDGE_PID')
        ...
        for epoch in (time.time(),time.time()-86400):
            ...
            for raw in cache.read_text().splitlines():
                try:
                    ...
                    continue  # a listener may still be appending the final line
                if str(record.get('m'))==message_id:
                    cli=str(record.get('c') or '')
                    if not cli or cli==message_id or not int(record.get('t') or 0):
                        raise SystemExit('FAIL_NATIVE_CLI_IDENTITY')
                    return {{'msgType':'webchat','msgId':message_id,'cliMsgId':cli,
                             'content':seed,'ownerId':own,'uidFrom':own,'ts':str(record['t'])}}
        time.sleep(.3)
    raise SystemExit('FAIL_NATIVE_QUOTE_CACHE_MISSING')

dm_quote=native_quote(dm_quote_id,dm_seed)
group_quote=native_quote(group_quote_id,group_seed)
...
with psycopg.connect(os.environ["DATABASE_URL"]) as conn:
    row=conn.execute(
        "SELECT count(*), COALESCE(bool_or(content=%s), false), COALESCE(bool_or(meta->>'quoted'='true' AND message_id ~ '^[0-9]+$'),false) FROM zalo_message_history WHERE thread_id=%s AND thread_type=%s AND event='delivered' AND COALESCE(meta->>'delivery_kind','result')!='gate' AND created_at >= to_timestamp(%s) AND meta->>'source_message_id'=%s",
...
print(json.dumps({{"count":int(row[0] or 0),"content_exact":bool(row[1]),"native_quoted":bool(row[2])}}))
"""
...
def inject(thread_id, thread_type, message_id, marker, quote):
...
with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
    ...
    futures=[
        pool.submit(inject,uid,"user",dm_source_id,dm_marker,dm_quote),
        pool.submit(inject,gid,"group",group_source_id,group_marker,group_quote),
    ]
...
while time.time()<deadline:
    ...
    dm_ok=dm_evidence.get("count")==1 and dm_evidence.get('native_quoted') is True
    group_ok=group_evidence.get("count")==1 and group_evidence.get('native_quoted') is True
    ...
    if dm_ok and group_ok and dm_content_exact and group_content_exact and thread_terminal(uid) and thread_terminal(gid):
...
if not settled or time.monotonic()-settled<6 or not dm_ok or not group_ok or not dm_content_exact or not group_content_exact or crossed:
    print(json.dumps({{
...
print(json.dumps({{
    ...
    "incoming_context":"injected","native_client_input_verified":False,
    "native_outbound_quote_ack":True,"terminal_settled":True,
    "dm_content_exact":dm_content_exact,"group_content_exact":group_content_exact,
}},separators=(",",":")))
PY
'''
