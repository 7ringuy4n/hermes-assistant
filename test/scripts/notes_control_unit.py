...
def main() -> int:
    ...
    memory_source = (ROOT / "architect" / "memory" / "memory-worker" / "app.py").read_text(encoding="utf-8")
    ...
    # Semantic relaxation is now model-owned and explicit, not a second SQL
    # lookup that silently discards a user's predicate. Behavior is covered by
    # memory_note_query_unit and scoped_records_unit.
    assert "fallback_clauses = date_clauses" not in memory_source
