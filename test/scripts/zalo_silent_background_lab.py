...
python3 - <<'PY'
import json,os,subprocess,time,urllib.request,urllib.error,urllib.parse,sys
...
text='1 phút nữa tìm thông tin thời tiết Hồ Chí Minh hiện tại trên web và ghi chú lại. Nội dung ghi chú cần có mã '+marker+'.'
...
with psycopg.connect(os.environ['DATABASE_URL']) as c:
 notes=c.execute("SELECT id,title,content FROM notes WHERE active AND scope_id=%s AND created_at>=to_timestamp(%s) AND metadata->>'source_message_id' LIKE %s",('zalo:user:'+os.environ['UID'],float(os.environ['START']),'schedule:'+os.environ['SID']+':%')).fetchall()
...
def evidence(sid):
 return json.loads(subprocess.check_output(['docker','exec','-e','UID='+uid,'-e','START='+str(started),'-e','MARKER='+marker,'-e','SID='+sid,zalo,'python3','-c',probe],text=True))
...
try:
 ...
 assert any(marker in n[2] for n in result['notes']), 'explicit user note identifier was omitted'
 ...
 terminal_since=None; deadline=time.time()+90
 while time.time()<deadline:
  assert evidence(sid)['delivered']==0, 'silent task sent a fire response'
  if thread_terminal(uid):
   terminal_since=terminal_since or time.monotonic()
   if time.monotonic()-terminal_since>=15: break
  else: terminal_since=None
  time.sleep(3)
 else: raise AssertionError('background note persisted without terminal completion')
finally:
 try:
  api('8110/v1/schedules/'+sid,method='DELETE')
 except urllib.error.HTTPError as exc:
  if exc.code != 404: raise
 # Only this unique request's marker-owned note IDs may be cleaned up.
 for note in evidence(sid)['notes']:
  api('8095/v1/notes/'+urllib.parse.quote(str(note[0]),safe='')+'?scope_id='+urllib.parse.quote('zalo:user:'+uid,safe=''),method='DELETE')
 assert not evidence(sid)['notes'], 'owned note cleanup incomplete'
print(json.dumps({'ok':True,'checks':3,'notes':len(result['notes']),'fire_deliveries':0,'owned_cleanup':True},separators=(',',':')),flush=True)
PY
